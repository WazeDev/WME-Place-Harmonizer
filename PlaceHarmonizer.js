...
function runPH() {
	...
	function setupKBShort() {
		registerKeyShortcut("harmonizePlace", "Harmonize Place", harmonizePlace, {"C+e": "harmonizePlace"});
	}
   ...
   function alertMessage(messageNum) {
	...
	function harmonizePlaceGo() {
		...
		sidebarMessage = ["Place is formatted and locked."];
		severity = 0;
...
// USA Regional switches
			...
			switch (addr.state.name) {
				case "Arkansas":case "Louisiana":case "Mississippi":case "Oklahoma":
					region = "SCR";
					break;
				case "Alaska":case "Idaho":case "Montana":case "Oregon":case "Washington":case "Wyoming":
					region = "NWR";
					break;
				case "Arizona":case "California":case "Colorado":case "Hawaii":case "Nevada":case "New Mexico":case "Utah":
					region = "SWR";
					break;
				case "Iowa":case "Kansas":case "Minnesota":case "Missouri":case "Nebraska":case "North Dakota":case "South Dakota":
					region = "PLN";
					break;
				case "Texas":
					region = "TX";
					break;
				case "Illinois":case "Indiana":case "Michigan":case "Ohio":case "Wisconsin":
					region = "GLR";
					break;
				case "Kentucky":case "North Carolina":case "South Carolina":case "Tennessee":
					region = "SAT";
					break;
				case "Alabama":case "Florida":case "Georgia":
					region = "SER";
					break;
				case "Connecticut":case "Maine":case "Massachusetts":case "New Hampshire":case "Rhode Island":case "Vermont":
					region = "NEW";
					break;
				case "Delaware":case "New Jersey":case "New York":case "Pennsylvania":
					region = "NOR";
					break;
				case "District of Columbia":case "Maryland":case "Virginia":case "West Virginia":
					region = "MAR";
					break;
				default:
					region = "Unknown";
...
// Place Harmonization 
	...
	//	Non Gas Stations:
			...
			nameShort.toUpperCase() == "AVISRENTAL" || nameShort.toUpperCase() == "AVISRENTALCAR" ) {
				...
				regList = "SAT, SER";  if (checkRegion(regList,region)) {
					tempCat = ["FAST_FOOD"];  desServ = ["RESTROOMS","CREDIT_CARDS","AIR_CONDITIONING","PARKING_FOR_CUSTOMERS","TAKE_AWAY","WHEELCHAIR_ACCESSIBLE"];
		...
		// BANKS: 
				...
				if ( !(categories.indexOf("BANK_FINANCIAL") > -1) ) { sidebarMessage.push(alertList[0]); severity = 3; }
				else {
...
// Place Area check
			...
			if ( region === "SER" && item.isPoint() && (tempCat.indexOf("JUNCTION_INTERCHANGE") > -1 || tempCat.indexOf("SEA_LAKE_POOL") > -1 || tempCat.indexOf("RIVER_STREAM") > -1 ||
...
// User alerts for potentially confusing places
			...
			if ( tempName == "Home" || tempName == "Casa" || tempName == "My Home" || tempName == "Mi Casa" ) {
				sidebarMessage.push("The place name suggests a residential place.");
        		severity = Math.max(2, severity);
			}
			if ( tempName == "FedEx" ) {
			...
			if ( tempName == "IBM Southeast EFCU" ) {
				sidebarMessage.push("Please add the suffix ' - LOCATION' to the primary name as found on IBMSEFCU's website, ibmsecu.org");
        		severity = Math.max(2, severity);
			}
			...
			if ( brandSwap == 1 ) {
				sidebarMessage = "The gas brand didn't match the primary name.  Check that the brand is indeed the current brand and verify all scripted changes.";
			...
			if (severity == 0) {
				$('<div id="WMEPH_logger_warn">').css("width", "290").css("background-color", "rgb(36, 172, 36)").css("color", "white").css("font-size", "15px").css("font-weight", "bold").css("margin-left", "auto").css("margin-right", "auto").appendTo(".contents")
			}
			if (severity == 1) {
				$('<div id="WMEPH_logger_warn">').css("width", "290").css("background-color", "rgb(40, 40, 230)").css("color", "white").css("font-size", "15px").css("font-weight", "bold").css("margin-left", "auto").css("margin-right", "auto").appendTo(".contents")
			}
			if (severity == 2) {
				$('<div id="WMEPH_logger_warn">').css("width", "290").css("background-color", "rgb(217, 173, 42)").css("color", "white").css("font-size", "15px").css("font-weight", "bold").css("margin-left", "auto").css("margin-right", "auto").appendTo(".contents")
			}
			if (severity == 3) {
				$('<div id="WMEPH_logger_warn">').css("width", "290").css("background-color", "rgb(211, 48, 48)").css("color", "white").css("font-size", "15px").css("font-weight", "bold").css("margin-left", "auto").css("margin-right", "auto").appendTo(".contents")
			}
