...
(function main() {
  ...
  class Country {
    ...
    constructor(
      ...
      Pnh.processCategories(
        Pnh.processImportedDataColumn(allSpreadsheetData, categoryColumnIndex),
        this.categoryInfos
      );
  ...
  const PROD_DOWNLOAD_URL =
    "https://greasyfork.org/scripts/28690-wme-place-harmonizer/code/WME%20Place%20Harmonizer.user.js";
  ...
  let _searchResultsWindowSpecs = `"resizable=yes, top=${Math.round(
    window.screen.height * 0.1
  )}, left=${Math.round(window.screen.width * 0.3)}, width=${Math.round(
  ...
  const URLS = {
    ...
    usaPnh:
      "https://docs.google.com/spreadsheets/d/1-f-JTWY5UnBx-rFTa4qhyGMYdHBZWNirUTOgn222zMY/edit#gid=0",
    placesWiki: "https://wazeopedia.waze.com/wiki/USA/Places",
    restAreaWiki:
      "https://wazeopedia.waze.com/wiki/USA/Rest_areas#Adding_a_Place",
    uspsWiki: "https://wazeopedia.waze.com/wiki/USA/Places/Post_office",
  };
  class Region {
    static #defaultNewChainRequestEntryIds = [
      "entry.925969794",
      "entry.1970139752",
      "entry.1749047694",
    ];
    static #defaultApproveChainRequestEntryIds = [
      "entry.925969794",
      "entry.50214576",
      "entry.1749047694",
    ];
    ...
    constructor(
      ...
      this.#formId = formId;
      this.#newChainRequestEntryIds =
        newChainRequestEntryIds ?? Region.#defaultNewChainRequestEntryIds;
      this.#approveChainRequestEntryIds =
        approveChainRequestEntryIds ??
        Region.#defaultApproveChainRequestEntryIds;
    }

    #getFormUrl(entryIds, entryValues) {
      const entryValuesUrl = entryValues
        .map((value, idx) => `${entryIds[idx]}=${value}`)
        .join("&");
      return `https://docs.google.com/forms/d/${
        this.#formId
      }/viewform?${entryValuesUrl}`;
    }
  ...
  const TITLECASE_SETTINGS = {
    ignoreWords:
      "an|and|as|at|by|for|from|hhgregg|in|into|of|on|or|the|to|with".split(
        "|"
      ),
  ...
  const CATS_THAT_DONT_NEED_NAMES = [CAT.SEA_LAKE_POOL];
  ...
  const UPDATED_FIELDS = {
    ...
    address: {
      updated: false,
      selector:
        "#venue-edit-general div.address-edit-view div.full-address-container",
      tab: "general",
    },
    categories: {
      updated: false,
      selector:
        "#venue-edit-general > div.categories-control.form-group > wz-card",
      shadowSelector: "div",
      tab: "general",
    },
    description: {
    ...
    externalProvider: {
      updated: false,
      selector:
        "#venue-edit-general > div.external-providers-control.form-group > wz-list",
      tab: "general",
    },
    brand: {
      updated: false,
      selector: ".venue .brand .select2-container",
      tab: "general",
    },
    url: {
      updated: false,
    ...
    canExit: {
      updated: false,
      selector: '.venue label[for="can-exit-checkbox"]',
      tab: "more-info",
    },
    hasTBR: {
      updated: false,
      selector: '.venue label[for="has-tbr"]',
      tab: "more-info",
    },
    lotType: {
      updated: false,
      selector:
        "#venue-edit-more-info > form > div:nth-child(1) > wz-radio-group",
      tab: "more-info",
    },
    parkingSpots: {
      updated: false,
      selector:
        '#venue-edit-more-info wz-select[name="estimatedNumberOfSpots"]',
      shadowSelector: "#select-wrapper > div",
      tab: "more-info",
    },
    lotElevation: {
      updated: false,
      selector: ".venue .lot-checkbox",
      tab: "more-info",
    },
    ...
    evPaymentMethods: {
      updated: false,
      selector:
        "#venue-edit-general > div.charging-station-controls div.wz-multiselect > wz-card",
      shadowSelector: "div",
      tab: "general",
    },
    evCostType: {
      updated: false,
      selector:
        "#venue-edit-general > div.charging-station-controls > wz-select",
    ...
    init() {
      [
        ...
        this[propName] = {
          updated: false,
          selector: `.venue label[for="service-checkbox-${service}"]`,
          tab: "more-info",
        };
      });
    ...
    getTabElement(tabName) {
      ...
      const tabElements = document
        .querySelector("#edit-panel div.venue-edit-section > wz-tabs")
        ?.shadowRoot?.querySelectorAll(".wz-tab-label");
      if (tabElements) {
        return [...tabElements].filter(
          (elem) => elem.textContent === tabText
        )[0];
      }
      return null;
    },
      ...
      this.getFieldProperties()
        ...
        .forEach((prop) => {
          if (prop.shadowSelector) {
            $(
              document
                .querySelector(prop.selector)
                ?.shadowRoot?.querySelector(prop.shadowSelector)
  ...
  class PnhEntry {
    ...
    constructor(columnHeaders, rowString, country) {
      const parseResult = this.#parseSpreadsheetRow(
        columnHeaders,
        rowString,
        country
      );
    ...
    #parseSpreadsheetRow(columnHeaders, rowString, country) {
      ...
      try {
        ...
        const disabled =
          columnValues[columnHeaders.indexOf(Pnh.SSHeader.disable)].trim();
        if (disabled === "1") {
        ...
        columnHeaders.forEach((header, i) => {
          try {
              ...
              switch (header) {
                ...
                case Pnh.SSHeader.category1:
                  if (value) {
                    this.primaryCategory =
                      country.categoryInfos.getByName(value)?.id;
                    if (typeof this.primaryCategory === "undefined") {
                      result.warningMessages.push(
                        `Unrecognized primary category value: ${value}`
                      );
                    }
                  } else {
                    result.warningMessages.push(
                      "No primary category assigned. PNH entry will be ignored!"
                    );
                    this.invalid = true;
                  }
                  break;
                case Pnh.SSHeader.category2:
                  this.altCategories = value
                    ...
                    .map((catName) => {
                      ...
                      if (!cat) {
                        result.warningMessages.push(
                          `Unrecognized alternate category: ${catName}`
                        );
                      }
                      return cat;
                    })
                ...
                case Pnh.SSHeader.region:
                  if (value) {
                    ...
                    this.invalid = true;
                    result.warningMessages.push(
                      "No regions specified. PNH entry will be ignored!"
                    );
                  }
                  break;
                case Pnh.SSHeader.disable:
                  ...
                  if (value === "altName") {
                    ...
                    result.warningMessages.push(
                      `Unrecognized value in ${Pnh.SSHeader.disable} column: ${value}`
                    );
                  }
                  return;
                case Pnh.SSHeader.forcecat:
                  if (!value || value === "0") {
                    this.forceCategoryMatching =
                      Pnh.ForceCategoryMatchingType.NONE;
                  } else if (value === "1") {
                    this.forceCategoryMatching =
                      Pnh.ForceCategoryMatchingType.PRIMARY;
                  } else if (value === "2") {
                    this.forceCategoryMatching =
                      Pnh.ForceCategoryMatchingType.ANY;
                  } else {
                    result.warningMessages.push(
                      `Unrecognized value in ${Pnh.SSHeader.forcecat} column: ${value}`
                    );
                  }
                  break;
                    ...
                    value.forEach((specialCase) => {
                      ...
                      if ((match = specialCase.match(/^buttOn_(.*)/i))) {
                        ...
                        switch (scFlag) {
                          ...
                          default:
                            result.warningMessages.push(
                              `Unrecognized ph_specCase value: ${specialCase}`
                            );
                        }
                      } else if (
                        (match = specialCase.match(/^buttOff_(.+)/i))
                      ) {
                        const [, scFlag] = match;
                        switch (scFlag) {
                          case "addConvStore":
                        ...
                        (match = specialCase.match(/^checkLocalization<>(.+)/i))
                      ) {
                        const [, localizationString] = match;
                        this.localizationRegEx = new RegExp(
                          localizationString,
                          "g"
                        );
                      } else if (
                        (match = specialCase.match(/phone<>(.*?)<>/))
                      ) {
                        [, this.recommendedPhone] = match;
                      } else if (/keepName/g.test(specialCase)) {
                        this.keepName = true;
                      } else if (
                        (match = specialCase.match(/^optionAltName<>(.+)/i))
                      ) {
                        [, this.optionalAlias] = match;
                      } else if (/^closed$/i.test(specialCase)) {
                        this.chainIsClosed = true;
                      } else if (
                        (match = specialCase.match(/^brandParent(\d+)/))
                      ) {
                        try {
                          this.brandParentLevel = parseInt(match[1], 10);
                        } catch {
                          result.warningMessages.push(
                            `Invalid forceBrand value: ${specialCase}`
                          );
                        }
                        ...
                        (match = specialCase.match(/^regexNameMatch<>(.+)<>/i))
                      ) {
                        this.regexNameMatch = new RegExp(
                          match[1].replace(/\\/, "\\").replace(/<or>/g, "|"),
                          "i"
                        );
                        ...
                        try {
                          ...
                          result.warningMessages.push(
                            `Invalid ph_speccase lockAt value (must be between 1 and 6): ${specialCase}`
                          );
                        }
            ...
            result.warningMessages.push(
              `An unexpected error occurred while processing column: ${header}. PNH entry will be ignored.`
            );
          }
        }); // END ROW PROCESSING
        ...
        result.warningMessages.push(
          `An unexpected error occurred while parsing. PNH entry will be ignored! :\n${ex.toString()}`
        );
        this.disabled = true;
      }
    ...
    #buildSearchNameList(parseResult) {
      ...
      if (!parseResult.skipAltNameMatch) {
        ...
        newNameList = newNameList.concat(
          this.aliases.map((alias) => PnhEntry.#tighten(alias))
        );
      }
      ...
      if (!isNullOrWhitespace(parseResult.searchnamebase)) {
        ...
        newNameList = newNameList.concat(
          PnhEntry.#stripNonAlphaKeepCommas(parseResult.searchnamebase).split(
            ","
          )
        );

        if (!isNullOrWhitespace(parseResult.searchnamemid)) {
          let pnhSearchNameMid = PnhEntry.#stripNonAlphaKeepCommas(
            parseResult.searchnamemid
          ).split(",");
          if (pnhSearchNameMid.length > 1) {
            // if there are more than one mid terms, it adds a permutation of the first 2
            pnhSearchNameMid = pnhSearchNameMid.concat([
              pnhSearchNameMid[0] + pnhSearchNameMid[1],
              pnhSearchNameMid[1] + pnhSearchNameMid[0],
            ]);
          }
        ...
        if (!isNullOrWhitespace(parseResult.searchnameend)) {
          const pnhSearchNameEnd = PnhEntry.#stripNonAlphaKeepCommas(
            parseResult.searchnameend
          ).split(",");
      ...
      if (categoryInfo) {
        ...
        appendWords.forEach((word) => {
          newNameList = newNameList.concat(
            newNameList.map((name) => name + word)
          );
        });
      }

      // Add entries for word/spelling variations
      Pnh.WORD_VARIATIONS.forEach((variationsList) =>
        addSpellingVariants(newNameList, variationsList)
      );
    ...
    getMatchInfo(
      ...
      if (this.regexNameMatch) {
        ...
        //     }
        /* } else */ if (
          searchNameList.includes(name) ||
          searchNameList.includes(venueNameNoNum)
        ) {
      ...
      if (nameMatch) {
        ...
        if (
          categories[0] === CAT.GAS_STATION ||
          PNHPriCat === CAT.GAS_STATION
        ) {
        ...
        matchInfo.isMatch =
          (PNHForceCat === Pnh.ForceCategoryMatchingType.PRIMARY &&
            categories.indexOf(PNHPriCat) === 0) ||
          // Name and any category match
          (PNHForceCat === Pnh.ForceCategoryMatchingType.ANY &&
            categories.includes(PNHPriCat)) ||
          // Name only match
  ...
  class Pnh {
    ...
    static SSHeader = Object.freeze({
      ...
      toValueArray: () =>
        Object.values(Pnh.SSHeader).filter((v) => typeof v === "string"),
    });
    ...
    static findMatch(
      ...
      if (
        country !== PNH_DATA.USA.countryCode &&
        country !== PNH_DATA.CAN.countryCode
      ) {
        return ["NoMatch"];
      }
      ...
      /** @type {PnhEntry[]} */
      const pnhData = closedChainsOnly
        ? PNH_DATA[country].closedChains
        : PNH_DATA[country].pnh;
      ...
      for (let pnhIdx = 0; pnhIdx < pnhData.length; pnhIdx++) {
        ...
        const matchInfo = pnhEntry.getMatchInfo(
          name,
          state2L,
          region3L,
          country,
    ...
    static #validatePnhSSColumnHeaders(headers) {
      ...
      headers.forEach((header) => {
        ...
        if (
          ...
          console.warn(
            `WMEPH: Unexpected column header found in PNH spreadsheet: ${header}`
          );
        }
      });
      ...
      expectedHeaders.forEach((header) => {
        if (!headers.includes(header)) {
          console.error(
            `WMEPH: Column header missing from PNH spreadsheet data: ${header}`
          );
          valid = false;
        }
      });
    ...
    static processPnhSSRows(allData, columnIndex, country) {
      ...
      if (!Pnh.#validatePnhSSColumnHeaders(columnHeaders)) {
        throw new Error(
          "WMEPH: WMEPH exiting due to missing spreadsheet column headers."
        );
      }
      return rows
        .map((row) => new PnhEntry(columnHeaders, row, country))
        .filter((entry) => !entry.disabled && !entry.invalid);
    }

    static processImportedDataColumn(allData, columnIndex) {
      return allData
        .filter((row) => row.length >= columnIndex + 1)
        .map((row) => row[columnIndex]);
    }

    static #getSpreadsheetUrl(id, range, key) {
      return `https://sheets.googleapis.com/v4/spreadsheets/${id}/values/${range}?${dec(
        key
      )}`;
    }
    ...
    static downloadPnhData() {
      ...
      return new Promise((resolve, reject) => {
        ...
        $.getJSON(url)
          .done((res) => {
            ...
            if (values[0][0].toLowerCase() === "obsolete") {
              WazeWrap.Alerts.error(
                SCRIPT_NAME,
                "You are using an outdated version of WMEPH that doesn't work anymore. Update or disable the script."
              );
              return;
            }
            ...
            PNH_DATA.USA = new Country("USA", "USA", values, 3, 0, {
              NWR: new Region(
                "NWR",
                "1hv5hXBlGr1pTMmo4n3frUx1DovUODbZodfDBwwTc7HE"
              ),
              SWR: new Region(
                "SWR",
                "1Qf2N4fSkNzhVuXJwPBJMQBmW0suNuy8W9itCo1qgJL4"
              ),
              HI: new Region(
                "HI",
                "1K7Dohm8eamIKry3KwMTVnpMdJLaMIyDGMt7Bw6iqH_A",
                null,
                ["entry.1497446659", "entry.50214576", "entry.1749047694"]
              ),
              PLN: new Region(
                "PLN",
                "1ycXtAppoR5eEydFBwnghhu1hkHq26uabjUu8yAlIQuI"
              ),
              SCR: new Region(
                "SCR",
                "1KZzLdlX0HLxED5Bv0wFB-rWccxUp2Mclih5QJIQFKSQ"
              ),
              GLR: new Region(
                "GLR",
                "19btj-Qt2-_TCRlcS49fl6AeUT95Wnmu7Um53qzjj9BA"
              ),
              SAT: new Region(
                "SAT",
              ...
              ATR: new Region(
                "ATR",
                "1v7JhffTfr62aPSOp8qZHA_5ARkBPldWWJwDeDzEioR0"
              ),
              NER: new Region(
                "NER",
                "1UgFAMdSQuJAySHR0D86frvphp81l7qhEdJXZpyBZU6c"
              ),
              NOR: new Region(
                "NOR",
                "1iYq2rd9HRd-RBsKqmbHDIEBGuyWBSyrIHC6QLESfm4c"
              ),
              MAR: new Region(
                "MAR",
                "1PhL1iaugbRMc3W-yGdqESoooeOz-TJIbjdLBRScJYOk"
              ),
            });
            ...
            const WMEPHuserList = Pnh.processImportedDataColumn(
              values,
              4
            )[1].split("|");
            ...
            _wmephDevList = [];
            _wmephBetaList = [];
            for (let ulix = 1; ulix < betaix; ulix++)
              _wmephDevList.push(WMEPHuserList[ulix].toLowerCase().trim());
            for (let ulix = betaix + 1; ulix < WMEPHuserList.length; ulix++)
              _wmephBetaList.push(WMEPHuserList[ulix].toLowerCase().trim());
          ...
          .fail((res) => {
            const message =
              res.responseJSON && res.responseJSON.error
                ? res.responseJSON.error
                : "See response error message above.";
    ...
    static #downloadPnhModerators() {
      ...
      return new Promise((resolve) => {
        const url = Pnh.#getSpreadsheetUrl(
          Pnh.#SPREADSHEET_ID,
          Pnh.#SPREADSHEET_MODERATORS_RANGE,
          Pnh.#API_KEY
        );
        ...
        if (iRow === 0) {
          ...
          row.forEach((value, iCol) => {
            ...
            switch (headerValue) {
              ...
              case "ps_wheelchair":
                if (value) {
                  categoryInfo.services.push({
                    wmeId: wmeServiceIds[iCol],
                    pnhKey: pnhServiceKeys[iCol],
                  });
                }
                break;
              case "":
              ...
              default:
                throw new Error(
                  `WMEPH: Unexpected category data from PNH sheet: ${headerValue}`
                );
            }
          });
  ...
  const SHORTCUT = {
    ...
    add(shortcutCombo, callback, opt) {
      ...
      this.allShortcuts[shortcutCombo] = {
        callback: func,
        target: ele,
        event: opt.type,
      };
  ...
  function addPURWebSearchButton() {
    ...
    purLayerObserver.observe($("#map #panel-container")[0], {
      childList: true,
      subtree: true,
    });
          ...
          const $btn = $("<div>")
            ...
            .append(
              $("<button>", {
                ...
                title:
                  "Search Google for this place. Do not copy info from 3rd party sources!",
    ...
    function buildSearchUrl(searchName, address) {
      ...
      searchName = encodeURIComponent(
        searchName + (address.length > 0 ? `, ${address}` : "")
      );
  ...
  function addSpellingVariants(nameList, spellingVariantList) {
    for (
      let spellingOneIdx = 0;
      spellingOneIdx < spellingVariantList.length;
      spellingOneIdx++
    ) {
      const spellingOne = spellingVariantList[spellingOneIdx];
      const namesToCheck = nameList.filter((name) =>
        name.includes(spellingOne)
      );
      for (
        let spellingTwoIdx = 0;
        spellingTwoIdx < spellingVariantList.length;
        spellingTwoIdx++
      ) {
  ...
  function clickGeneralTab() {
    ...
    const containerSelector =
      "#edit-panel > div > div.venue-feature-editor > div > div.venue-edit-section > wz-tabs";
  ...
  function backupWhitelistToLS(compress) {
    ...
    if (compress) {
      ...
      localStorage.setItem(
        WL_LOCAL_STORE_NAME_COMPRESSED + Math.floor(Date.now() / 1000),
        wlString
      );
    } else {
      localStorage.setItem(
        WL_LOCAL_STORE_NAME + Math.floor(Date.now() / 1000),
        wlString
      );
    }
  }
  ...
  function nudgeVenue(venue) {
    ...
    const action = new UpdateFeatureGeometry(
      venue,
      W.model.venues,
      venue.getGeometry(),
      newGeometry
    );
    const mAction = new MultiAction([action], {
      description: "Place nudged by WMEPH",
    });
  ...
  function deleteDupeLabel() {
    setTimeout(() => {
      ...
      if (
        typeof lastAction !== "undefined" &&
        lastAction.hasOwnProperty("object") &&
        lastAction.object.hasOwnProperty("state") &&
        lastAction.object.state === "Delete"
      ) {
        if (_dupeIDList.includes(lastAction.object.attributes.id)) {
          if (_dupeIDList.length === 2) {
            destroyDupeLabels();
          } else {
            const deletedDupe = _dupeLayer.getFeaturesByAttribute(
              "dupeID",
              lastAction.object.attributes.id
            );
            _dupeLayer.removeFeatures(deletedDupe);
            _dupeIDList.splice(
              _dupeIDList.indexOf(lastAction.object.attributes.id),
              1
            );
          }
          log("Deleted a dupe");
  ...
  function whitelistAction(venueID, wlKeyName) {
    ...
    if (!addressTemp.country) {
      WazeWrap.Alerts.error(
        SCRIPT_NAME,
        "Whitelisting requires an address. Enter the place's address and try again."
      );
      return false;
    }
  ...
  function wmephWhitelistCounter() {
    // eslint-disable-next-line camelcase
    localStorage.WMEPH_WLAddCount =
      parseInt(localStorage.WMEPH_WLAddCount, 10) + 1;
    if (localStorage.WMEPH_WLAddCount > 50) {
      WazeWrap.Alerts.warning(
        SCRIPT_NAME,
        "Don't forget to periodically back up your Whitelist data using the Pull option in the WMEPH settings tab."
      );
      // eslint-disable-next-line camelcase
      ...
      mutations.forEach((mutation) => {
        ...
        for (let i = 0; i < mutation.addedNodes.length; i++) {
          ...
          if (
            ...
            addedNode.querySelector(".tab-scroll-gradient")
          ) {
  ...
  function appendServiceButtonIconCss() {
    ...
    $("head").append(
      $("<style>", { type: "text/css" }).html(cssArray.join("\n"))
    );
  }
  ...
  function toggleXrayMode(enable) {
    localStorage.setItem(
      ...
      $("#layer-switcher-item_wmeph_x-ray_mode").prop("checked")
    );
    ...
    const commentRuleSymb =
      commentsLayer.styleMap.styles.default.rules[0].symbolizer;
    if (enable) {
      _layer.styleMap.styles.default.rules =
        _layer.styleMap.styles.default.rules.filter(
          (rule) => rule.wmephDefault !== "default"
        );
    ...
    const ruleGenerator = (value, symbolizer) =>
      new W.Rule({
    ...
    _layer.styleMap.styles.default.rules.push(
      ...[
        severity0,
        severityLock,
        severity1,
  ...
  function titleCase(str) {
    ...
    str = str
      .replace(/([A-Za-z\u00C0-\u017F][^\s-/]*) */g, (txt) => {
        ...
        if (/^([dDlL]e|[lL]a)[A-Z][a-zA-Z\u00C0-\u017F]{2,}/.test(txt)) {
          return (
            ...
            txt.charAt(1).toLowerCase() +
            txt.charAt(2) +
            txt.substr(3).toLowerCase()
          );
        }
        return txt === txt.toUpperCase() && !allCaps
          ? txt
          : txt.charAt(0).toUpperCase() + txt.substr(1).toLowerCase();
      })
      // Cap O'Reilley's, L'Amour, D'Artagnan as long as 5+ letters
      .replace(/\b[oOlLdD]'[A-Za-z']{3,}/g, (txt) =>
        txt === txt.toUpperCase() && !allCaps
          ? txt
          : txt.charAt(0).toUpperCase() +
            txt.charAt(1) +
            txt.charAt(2).toUpperCase() +
            txt.substr(3).toLowerCase()
      )
      ...
      .replace(/&\w+/g, (txt) =>
        txt === txt.toUpperCase() && !allCaps
          ? txt
          : txt.charAt(0) + txt.charAt(1).toUpperCase() + txt.substr(2)
      )
      ...
      .replace(/[^ ]+/g, (txt) => {
        ...
        return (
          TITLECASE_SETTINGS.specWords.find(
            (specWord) => specWord.toUpperCase() === txtUC
          ) || txt
        );
      })
    ...
    macIndexes.forEach((idx) => {
      str =
        str.substr(0, idx + 3) +
        str.substr(idx + 3, 1).toUpperCase() +
        str.substr(idx + 4);
    });
  ...
  function replaceLetters(number) {
    ...
    return number.replace(/[A-Z]/g, (letter) =>
      conversionMap.findKey((re) => re.test(letter))
    );
  }
    ...
    if (checkboxChecked === servBtn.checked && checkboxChecked !== checked) {
      ...
      if (actions) {
        for (let i = 0; i < actions.length; i++) {
          ...
          if (
            existingAction.newAttributes &&
            existingAction.newAttributes.services
          ) {
  ...
  function normalizeURL(url, makeLowerCase = true) {
    ...
    if (!url || url.trim().length === 0 || !/(^https?:\/\/)?\w+\.\w+/.test(url))
      url = BAD_URL;
    return url;
  }
  ...
  function harmonizePlace() {
    ...
    if (IS_BETA_VERSION && !USER.isBetaUser) {
      WazeWrap.Alerts.error(
        SCRIPT_NAME,
        "Please sign up to beta-test this script version.<br>Contact MapOMatic or Tonestertm in Discord, or post in the WMEPH forum thread. Thanks."
      );
      return;
    }
  ...
  class WLFlag extends FlagBase {
    ...
    get severity() {
      return this.constructor.isWhitelisted(this.args)
        ? SEVERITY.GREEN
        : super.severity;
    }
    set severity(value) {
  ...
  const Flag = {
    ...
    FullAddressInference: class extends FlagBase {
      ...
      static defaultMessage =
        "Missing address was inferred from nearby segments. Verify the address and run WMEPH again.";
          ...
          if (!args.addr.state || !args.addr.country) {
            if (sdk.Map.getZoomLevel() < 4) {
              if ($("#WMEPH-EnableIAZoom").prop("checked")) {
                ...
                WazeWrap.Alerts.error(
                  SCRIPT_NAME,
                  "No address and the state cannot be determined. Please zoom in and rerun the script. " +
              ...
              if (inferredAddress?.state && inferredAddress.country) {
                if ($("#WMEPH-AddAddresses").prop("checked")) {
                  ...
                  ![CAT.JUNCTION_INTERCHANGE].includes(args.categories[0])
                ) {
                ...
                WazeWrap.Alerts.error(
                  SCRIPT_NAME,
                  "This place has no address data and the address cannot be inferred from nearby segments. Please edit the address and run WMEPH again."
                );
                result = { exit: true }; // Don't bother returning a Flag. This will exit the rest of the harmonizePlaceGo function.
              }
            }
    ...
    PlaIsPublic: class extends FlagBase {
      static get defaultMessage() {
        ...
        msg += [
          ...
          .map((btnInfo) =>
            $("<button>", {
              class: "wmeph-pla-lot-type-btn btn btn-default btn-xs wmeph-btn",
              "data-lot-type": btnInfo[0],
            })
              .text(btnInfo[1])
              .prop("outerHTML")
          )
      ...
      postProcess() {
        $(".wmeph-pla-lot-type-btn").click((evt) => {
          ...
          const categoryAttrClone = JSON.parse(
            JSON.stringify(this.args.venue.attributes.categoryAttributes)
          );
          categoryAttrClone.PARKING_LOT.parkingType = lotType;
          UPDATED_FIELDS.lotType.updated = true;
          addUpdateAction(
            this.args.venue,
            { categoryAttributes: categoryAttrClone },
            null,
            true
          );
        });
    ...
    PlaNameMissing: class extends FlagBase {
      ...
      static get defaultMessage() {
        return `Name is missing. ${
          USER.rank < 3 ? "Request an R3+ lock" : "Lock to 3+"
        } to confirm unnamed parking lot.`;
      }
      noLock = true;
          ...
          if (name) {
            ...
            const re =
              state === "Quebec"
                ...
                : /\b((park[ -](and|&|'?n'?)[ -]ride)|parking|lot|garage|ramp)\b/i;
            if (!re.test(name)) {
              return true;
            }
          }
    ...
    HoursOverlap: class extends FlagBase {
      static defaultSeverity = SEVERITY.RED;
      static defaultMessage =
        "Overlapping hours of operation. Place might not save.";
    ...
    UnmappedRegion: class extends WLFlag {
      ...
      get noLock() {
        return Flag.UnmappedRegion.#getRareCategoryInfos(this.args).some(
          (categoryInfo) =>
            (categoryInfo.id === CAT.OTHER &&
              Flag.UnmappedRegion.#regionsToFlagOther.includes(
                this.args.regionCode
              ) &&
              !this.args.isLocked) ||
      ...
      constructor(args) {
        ...
        Flag.UnmappedRegion.#getRareCategoryInfos(args).forEach(
          (categoryInfo) => {
            if (categoryInfo.id === CAT.OTHER) {
              if (
                Flag.UnmappedRegion.#regionsToFlagOther.includes(args.region) &&
                !args.isLocked
              ) {
        ...
        if (!args.highlightOnly) {
          ...
          if (categoryNames.length === 1) {
            messages.push(
              `The <b>${categoryNames[0]}</b> category is usually not mapped in this region.`
            );
          } else if (categoryNames.length > 1) {
            messages.push(
              `These categories are usually not mapped in this region: ${categoryNames
                .map((name) => `<b>${name}</b>`)
                .join(", ")}`
            );
          }
          if (addOtherMessage) {
            messages.push(
              "The <b>Other</b> category should only be used if no other category applies. " +
        ...
        return args.categories
          ...
          .filter((pnhCategoryInfo) => {
            ...
            if (
              ...
              rareLocalities.includes(args.countryCode)
            ) {
              if (
                pnhCategoryInfo.id === CAT.OTHER &&
                this.#regionsToFlagOther.includes(args.region)
              ) {
                if (!args.isLocked) {
                  return true;
                }
    ...
    RestAreaNoTransportation: class extends ActionFlag {
      static defaultSeverity = SEVERITY.YELLOW;
      static defaultMessage =
        "Rest areas should not use the Transportation category.";
    ...
    RestAreaGas: class extends FlagBase {
      static defaultSeverity = SEVERITY.RED;
      static defaultMessage =
        "Gas stations at Rest Areas should be separate area places.";
    ...
    EVChargingStationWarning: class extends FlagBase {
      ...
      static venueIsFlaggable(args) {
        return (
          !args.highlightOnly && args.categories.includes(CAT.CHARGING_STATION)
        );
      }
    },
    EVCSAltNameMissing: class extends ActionFlag {
      static defaultSeverity = SEVERITY.BLUE;
      static defaultMessage =
        'Public and restricted EV charging stations should have an alternate name of "EV Charging Station"';
    ...
    EVCSPriceMissing: class extends FlagBase {
      ...
      postProcess() {
        $(".wmeph-evcs-cost-type-btn").click((evt) => {
          const selectedValue = $(evt.currentTarget)
            .attr("id")
            .replace("wmeph_", "");
          let attrClone;
          if (this.args.venue.attributes.categoryAttributes) {
            attrClone = JSON.parse(
              JSON.stringify(this.args.venue.attributes.categoryAttributes)
            );
          } else {
            attrClone = {};
    ...
    GasMismatch: class extends WLFlag {
      ...
      static defaultMessage =
        '<a href="https://wazeopedia.waze.com/wiki/USA/Places/Gas_station#Name" target="_blank" class="red">' +
      ...
      static venueIsFlaggable(args) {
        ...
        if (args.categories[0] === CAT.GAS_STATION && args.brand) {
          const compressedName = (args.nameBase + args.nameSuffix ?? "")
            .toUpperCase()
            .replace(/[^a-zA-Z0-9]/g, "");
          ...
          const compressedBrands = [
            args.brand.toUpperCase().replace(/[^a-zA-Z0-9]/g, ""),
          ];
          ...
          if (
            !compressedBrands.some((compressedBrand) =>
              compressedName.includes(compressedBrand)
            )
          ) {
            return true;
          }
    ...
    GasUnbranded: class extends FlagBase {
      ...
      static defaultSeverity = SEVERITY.RED;
      static defaultMessage =
        '"Unbranded" should not be used for the station brand. Change to the correct brand or delete the brand.';
    ...
    GasMkPrim: class extends ActionFlag {
      ...
      static defaultButtonTooltip =
        "Make the Gas Station category the primary category.";
    ...
    HotelMkPrim: class extends WLActionFlag {
      ...
      action() {
        ...
        const categories = insertAtIndex(
          this.args.venue.attributes.categories.slice(),
          CAT.HOTEL,
          0
        );
        addUpdateAction(this.args.venue, { categories }, null, true);
        ...
        if (!this.isWhitelisted(args)) {
          ...
          if (
            (args.categories.includes(CAT.HOSPITAL_URGENT_CARE) ||
              args.categories.includes(CAT.DOCTOR_CLINIC)) &&
            (containsAny(testNameWords, Pnh.ANIMAL_FULL_MATCH) ||
              Pnh.ANIMAL_PART_MATCH.some((match) => testName.includes(match)))
          ) {
            return true;
          }
        }
    ...
    NotASchool: class extends WLFlag {
      ...
      static venueIsFlaggable(args) {
        if (!this.isWhitelisted(args)) {
          ...
          if (
            args.categories.includes(CAT.SCHOOL) &&
            (containsAny(testNameWords, Pnh.SCHOOL_FULL_MATCH) ||
              Pnh.SCHOOL_PART_MATCH.some((match) => testName.includes(match)))
          ) {
            return true;
          }
        }
    ...
    AreaNotPoint: class extends WLActionFlag {
      ...
      static #hasCollegeInName(name) {
        if (!this.#collegeAbbrRegExps) {
          this.#collegeAbbrRegExps = COLLEGE_ABBREVIATIONS.map(
            (abbr) => new RegExp(`\\b${abbr}\\b`, "g")
          );
        }
      ...
      action() {
        ...
        W.model.actionManager.add(
          new UpdateFeatureGeometry(
            venue,
            venue.model.venues,
            venue.getOLGeometry(),
            venue.getPolygonGeometry()
          )
        );
        harmonizePlaceGo(venue, "harmonize");
      }
    ...
    HnMissing: class extends WLActionFlag {
      ...
      get message() {
        let msg =
          `No HN: <input type="text" id="${
            Flag.HnMissing.#TEXTBOX_ID
          }" autocomplete="off" ` +
      ...
      constructor(args) {
        ...
        if (
          args.state2L === "PR" ||
          args.categories[0] === CAT.SCENIC_LOOKOUT_VIEWPOINT
        ) {
          severity = SEVERITY.GREEN;
          showWL = false;
      ...
      action() {
        ...
        if (hnTemp > 0 && hnTemp < 1000000) {
          ...
          Flag.HnMissing.#getTextbox()
            .css({ backgroundColor: "#FDD" })
            .attr("title", "Must be a number between 0 and 1000000");
        }
      }
      ...
      static venueIsFlaggable(args) {
        if (!this.isWhitelisted(args) && _dupeHNRangeList.length > 3) {
          ...
          for (dhnix = 0; dhnix < _dupeHNRangeList.length; dhnix++) {
            dupeHNRangeListSorted.push(
              _dupeHNRangeList[_dupeHNRangeDistList.sortIndices[dhnix]]
            );
          }
          ...
          for (dhnix = 0; dhnix < dupeHNRangeListSorted.length; dhnix++) {
            arrayHNRatio.push(
              Math.abs(
                (parseInt(args.currentHN, 10) - dupeHNRangeListSorted[dhnix]) /
                  _dupeHNRangeDistList[dhnix]
              )
            );
          }
          sortWithIndex(arrayHNRatio);
          // Examine either the median or the 8th index if length is >16
          const arrayHNRatioCheckIX = Math.min(
            Math.round(arrayHNRatio.length / 2),
            8
          );
          if (arrayHNRatio[arrayHNRatioCheckIX] > 1.4) {
            ...
            logDev(
              `HN Ratio Score: ${
                arrayHNRatio[Math.round(arrayHNRatio.length / 2)]
              }`
            );
            return true;
          }
      ...
      action() {
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            const elem = document
              .querySelector(
                "#venue-edit-general > div:nth-child(1) > div > div > wz-card > form > div:nth-child(2) > div > wz-autocomplete"
              )
      ...
      action() {
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            const elem = document
              .querySelector(
                "#venue-edit-general > div:nth-child(1) > div > div > wz-card > form > div:nth-child(4) > wz-autocomplete"
              )
              .shadowRoot.querySelector("#text-input")
              .shadowRoot.querySelector("#id");
            elem.focus();
    ...
    BankType1: class extends FlagBase {
      static defaultSeverity = SEVERITY.RED;
      static defaultMessage =
        "Clarify the type of bank: the name has ATM but the primary category is Offices";

      static venueIsFlaggable(args) {
        return (
          (!args.pnhNameRegMatch ||
            (args.pnhNameRegMatch &&
              args.priPNHPlaceCat === CAT.BANK_FINANCIAL &&
              !args.pnhMatch.notABank)) &&
    ...
    BankBranch: class extends ActionFlag {
      ...
      action() {
        ...
        const newCategories = insertAtIndex(
          originalCategories,
          [CAT.BANK_FINANCIAL, CAT.ATM],
          0
        ); // Change to bank and atm cats
    ...
    StandaloneATM: class extends ActionFlag {
      ...
      static defaultButtonText = "Yes";
      static defaultButtonTooltip =
        "Is this a standalone ATM with no bank branch?";
      ...
      static venueIsFlaggable(args) {
        ...
        if (
          ...
          if (/\batm\b/gi.test(args.nameBase)) {
            flaggable =
              ixOffices === 0 || (ixBank === -1 && ixATM === -1) || ixBank > -1;
          } else if (ixBank > -1 || ixATM > -1) {
            flaggable =
              ixOffices === 0 ||
              (ixATM === 0 && ixBank === -1) ||
    ...
    BankCorporate: class extends ActionFlag {
      ...
      static venueIsFlaggable(args) {
        ...
        if (!args.priPNHPlaceCat) {
          flaggable =
            /\batm\b/gi.test(args.nameBase) &&
            args.categories.indexOf(CAT.OFFICES) === 0;
        } else if (
          args.priPNHPlaceCat === CAT.BANK_FINANCIAL &&
          !args.pnhMatch.notABank
        ) {
          flaggable =
            !containsAny(args.categories, [CAT.BANK_FINANCIAL, CAT.ATM]) &&
            !/\bcorporate offices\b/i.test(args.nameSuffix);
        }
        return flaggable;
      }
    ...
    CatPostOffice: class extends FlagBase {
      static defaultMessage = `The Post Office category is reserved for certain USPS locations. Please be sure to follow <a href="${URLS.uspsWiki}" style="color:#3a3a3a;" target="_blank">the guidelines</a>.`;
    ...
    ParentCategory: class extends WLFlag {
      ...
      get message() {
        ...
        const badCatInfos = this.args.categories
          .filter((category) =>
            Flag.ParentCategory.categoryIsDisallowedParent(category, this.args)
          )
        ...
        if (badCatInfos.length === 1) {
          ...
          msg =
            "These parent categories are usually not mapped in this region: ";
          msg += badCatInfos
            .map((catInfo) => `<b>${catInfo.name}</b>`)
            .join(", ");
        }
        return msg;
      }
      ...
      static categoryIsDisallowedParent(category, args) {
        ...
        return (
          localities.includes(args.state2L) ||
          localities.includes(args.region) ||
          localities.includes(args.countryCode)
        );
      }

      static venueIsFlaggable(args) {
        return args.categories.some((category) =>
          this.categoryIsDisallowedParent(category, args)
        );
      }
    },
    ...
    SuspectDesc: class extends WLFlag {
      static defaultSeverity = SEVERITY.YELLOW;
      static defaultMessage =
        "Description field might contain copyrighted info.";
    ...
    ResiTypeName: class extends WLFlag {
      ...
      static #likelyResidentialName(alphaName) {
        return /^((my|mi|moms|dads)?\s*(home|work|office|casa|house))|(mom|dad)$/i.test(
          alphaName
        );
      }
      ...
      static #possiblyResidentialName(alphaName, categories) {
        return (
          /('?s|my)\s+(house|home|work)/i.test(alphaName) &&
      ...
      static #venueIsFlaggable(
        ...
        return (
          preflaggable &&
          (likelyResidential ||
            this.#possiblyResidentialName(alphaName, categories))
        );
      }
      ...
      static eval(args) {
        ...
        if (preflaggable) {
          ...
          if (
            ...
            return new this(likelyResidential);
        }
        return null;
      }
    },
    Mismatch247: class extends FlagBase {
      static defaultSeverity = SEVERITY.YELLOW;
      static defaultMessage =
        "Hours of operation listed as open 24hrs but not for all 7 days.";
      ...
      static venueIsFlaggable(args) {
        return (
          ...
          (/^0?0:00$/.test(args.openingHours[0].toHour) ||
            args.openingHours[0].toHour === "23:59")
        );
      }
    },
    PhoneInvalid: class extends FlagBase {
      ...
      static venueIsFlaggable(args) {
        ...
        return (
          (args.highlightOnly && normalizedPhone !== args.phone) ||
    ...
    UrlAnalytics: class extends WLActionFlag {
      ...
      static defaultWLTooltip = "Whitelist existing URL";
      static URL_ANALYTICS_REGEX =
        /(?<=&|\?)(utm_|y_|(wtextnd)?source=|cmpid=|cid=|otppartnerid=|campaignid=|ref=|cmp=).*?(&|$)/gi;
      ...
      action() {
        if (!this.args.venue.isChargingStation()) {
          WazeWrap.Alerts.info(
            SCRIPT_NAME,
            "This is no longer a charging station. Please run WMEPH again.",
            false,
            false
          );
          return;
        ...
        if (network !== this.originalNetwork) {
          WazeWrap.Alerts.info(
            SCRIPT_NAME,
            "EV charging station network has changed. Please run WMEPH again.",
            false,
            false
          );
          return;
        ...
        const categoryAttrClone = JSON.parse(
          JSON.stringify(this.args.venue.getCategoryAttributes())
        );
    ...
    FormatUSPS: class extends FlagBase {
      ...
      static venueIsFlaggable(args) {
        return (
          args.isUspsPostOffice &&
          !this.isNameOk(
            this.getCleanNameParts(args.nameBase, args.nameSuffix).join(""),
            args.state2L,
            args.addr
          )
        );
        ...
        return state2L === "KY" ||
          (state2L === "NY" &&
            [
              ...
              "Brooklyn",
              "Staten Island",
            ].includes(addr.city?.attributes.name))
    ...
    MissingUSPSAlt: class extends ActionFlag {
      static defaultSeverity = SEVERITY.BLUE;
      static defaultMessage =
        'USPS post offices must have an alternate name of "USPS".';
      static defaultButtonText = "Add it";
...
id="WMEPH-zipAltNameAdd"autocomplete="off" style="font-size:0.85em;width:65px;padding-left:2px;color:#000;" title="Enter the ZIP code and click Add">`;
        ...
        if (zip) {
          if (/^\d{5}/.test(zip)) {
            ...
            if (!aliases.includes(zip)) {
              ...
              $input
                .css({ backgroundColor: "#FDD" })
                .attr("title", "Zip code alt name already exists");
            }
          } else {
            $input
              .css({ backgroundColor: "#FDD" })
              .attr("title", "Zip code format error");
          }
        }
      }
      ...
      postProcess() {
        ...
        const zipMatch = (
          this.args.nameBase + (this.args.nameSuffix ?? "")
        ).match(/\d{5}/);
        if (zipMatch) {
    ...
    MissingUSPSDescription: class extends WLFlag {
      static defaultSeverity = SEVERITY.BLUE;
      static defaultMessage = `The first line of the description for a <a href="${URLS.uspsWiki}" style="color:#3232e6" target="_blank">USPS post office</a> must be CITY, STATE(2-letter) ZIP, e.g. "Lexington, KY 40511"`;

      static WL_KEY = "missingUSPSDescription";
      static defaultWLTooltip =
      ...
      static venueIsFlaggable(args) {
        if (args.isUspsPostOffice) {
          ...
          return (
            !lines?.length || !/^.{2,}, [A-Z]{2}\s{1,2}\d{5}$/.test(lines[0])
          );
        }
        return false;
      }
    ...
    CatHotel: class extends FlagBase {
      constructor(args) {
        ...
        super(
          `Check hotel website for any name localization (e.g. ${pnhName} - Tampa Airport).`
        );
      }
      ...
      static venueIsFlaggable(args) {
        return (
          args.priPNHPlaceCat === CAT.HOTEL &&
          (args.nameBase + (args.nameSuffix || "")).toUpperCase() ===
            args.pnhMatch.name.toUpperCase()
        );
      }
    },
    LocalizedName: class extends WLFlag {
      ...
      get message() {
        return (
          this.args.pnhMatch.displaynote ||
          "Place needs localization information"
        );
      }

      static venueIsFlaggable(args) {
        return (
          args.pnhMatch.localizationRegEx &&
          !args.pnhMatch.localizationRegEx.test(
    ...
    SpecCaseMessage: class extends FlagBase {
      ...
      constructor(args) {
        ...
        let isRivian = false;
        const isTesla =
          Flag.SpecCaseMessage.#teslaSC.test(message) &&
          Flag.SpecCaseMessage.#teslaDC.test(message);
        if (isTesla) {
          ...
          isRivian =
            Flag.SpecCaseMessage.#rivianAN.test(message) &&
            Flag.SpecCaseMessage.#rivianW.test(message);
          if (isRivian) {
            message = message.replace(
    ...
    PnhCatMess: class extends ActionFlag {
      ...
      static eval(venue, pnhCategoryInfo, categories, highlightOnly) {
        return this.#venueIsFlaggable(highlightOnly, pnhCategoryInfo)
          ? new this(venue, pnhCategoryInfo, categories)
          : null;
      }
    ...
    ExtProviderMissing: class extends ActionFlag {
      ...
      action2() {
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            const elem = document
              .querySelector("div.external-provider-edit-form wz-autocomplete")
              .shadowRoot.querySelector("wz-text-input")
    ...
    UrlMissing: class extends WLActionFlag {
      ...
      static get defaultMessage() {
        return (
          `No URL: <input type="text" id="${
            Flag.UrlMissing.#TEXTBOX_ID
          }" autocomplete="off"` +
          ' style="font-size:0.85em;width:100px;padding-left:2px;color:#000;">'
        );
      }
      ...
      static isWhitelisted(args) {
        return (
          ...
          PRIMARY_CATS_TO_FLAG_GREEN_MISSING_PHONE_URL.includes(
            args.categories[0]
          ) ||
          ANY_CATS_TO_FLAG_GREEN_MISSING_PHONE_URL.some((category) =>
    ...
    BadAreaCode: class extends WLActionFlag {
      ...
      get message() {
        return (
          ...
          `style="font-size:0.85em;width:100px;padding-left:2px;color:#000;" value="${
            this.args.phone || ""
          }">`
        );
      }
      ...
      action() {
        const newPhone = normalizePhone(
          $("#WMEPH-PhoneAdd").val(),
          this.args.outputPhoneFormat
        );
        if (newPhone === BAD_PHONE) {
          $("input#WMEPH-PhoneAdd")
            .css({ backgroundColor: "#FDD" })
            .attr("title", "Invalid phone # format");
        } else {
    ...
    AddRecommendedPhone: class extends WLActionFlag {
      ...
      static venueIsFlaggable(args) {
        return (
          args.recommendedPhone &&
          !this.isWhitelisted(args) &&
          args.recommendedPhone !== BAD_PHONE &&
          args.recommendedPhone !==
            normalizePhone(args.phone, args.outputPhoneFormat)
        );
      }
      ...
      action() {
        addUpdateAction(
          this.args.venue,
          { phone: this.args.recommendedPhone },
          null,
          true
        );
      }
    ...
    NoHours: class extends WLFlag {
        ...
        return $("<span>").append(
          ...
          !alwaysOpen
            ? $("<input>", {
                ...
                id: "WMEPH_noHours",
                title: `Add pasted hours${
                  hasExistingHours ? " to existing hours" : ""
                }`,
                type: "button",
                value: "Add hours",
      ...
      applyHours(replaceAllHours) {
        ...
        logDev(pasteHours);
        pasteHours += !replaceAllHours
          ? `,${getOpeningHours(this.args.venue).join(",")}`
          : "";
        ...
        if (
          parseResult.hours &&
          !parseResult.overlappingHours &&
          !parseResult.sameOpenAndCloseTimes &&
          !parseResult.parseError
        ) {
          logDev(parseResult.hours);
          addUpdateAction(
            this.args.venue,
            { openingHours: parseResult.hours },
            null,
            true
          );
      ...
      static #getDaysString(days) {
        ...
        if (groupString.length === 1 && groupString[0] === "Mon–Sun")
          return "Every day";
      ...
      static #getHoursStringArray(hoursObjects) {
        ...
        const closedDays = [1, 2, 3, 4, 5, 6, 7].filter(
          (day) => !daysWithHours.includes(day)
        );
        if (closedDays.length) {
          outputArray.push(
            `${this.#getDaysString(closedDays)}:&nbsp&nbspCLOSED`
          );
        }
        return outputArray;
      }
      ...
      postProcess() {
        if (this.args.openingHours.length) {
          const hoursStringArray = Flag.NoHours.#getHoursStringArray(
            this.args.openingHours
          );
        ...
        function resetHoursEntryHeight() {
          ...
          setTimeout(() => {
            ...
            const height =
              lineCount * 18 +
              6 +
              (elem.scrollWidth > elem.clientWidth ? 20 : 0);
        ...
        $("#WMEPH-HoursPaste").after(
          $("<i>", {
            ...
            class: "fa fa-paste",
            style:
              "font-size: 17px;position: relative;vertical-align: top;top: 2px;right: -5px;margin-right: 3px;color: #6c6c6c;cursor: pointer;",
    ...
    OldHours: class extends ActionFlag {
      ...
      static #parentCategoriesToCheck = [
        CAT.SHOPPING_AND_SERVICES,
        CAT.FOOD_AND_DRINK,
        CAT.CULTURE_AND_ENTERTAINEMENT,
      ];
      ...
      static #initializeCategoriesToCheck(pnhCategoryInfos) {
        if (!this.#categoriesToCheck) {
          this.#categoriesToCheck = pnhCategoryInfos
            .toArray()
            .filter((pnhCategoryInfo) =>
              this.#parentCategoriesToCheck.includes(pnhCategoryInfo.parent)
            )
      ...
      static get defaultMessage() {
        return `Parking cost: ${[
          ...
          .map((btnInfo) =>
            $("<button>", {
              id: `wmeph_${btnInfo[0]}`,
              class: "wmeph-pla-cost-type-btn btn btn-default btn-xs wmeph-btn",
              title: btnInfo[2],
            })
              .text(btnInfo[1])
              .css({
    ...
    PlaPaymentTypeMissing: class extends ActionFlag {
      static defaultSeverity = SEVERITY.BLUE;
      static defaultMessage =
        'Parking isn\'t free. Select payment type(s) from the "More info" tab. ';
    ...
    PlaSpaces: class extends FlagBase {
      static get defaultMessage() {
        const msg =
          "# of parking spaces is set to 1-10.<br><b><i>If appropriate</i></b>, select another option:";
        const $btnDiv = $("<div>");
        let btnIdx = 0;
        [
          ["R_11_TO_30", "11-30"],
          ...
          $btnDiv.append(
            $("<button>", {
              id: `wmeph_${btnInfo[0]}`,
              class: "wmeph-pla-spaces-btn btn btn-default btn-xs wmeph-btn",
            })
              .text(btnInfo[1])
              .css({
                padding: "3px",
    ...
    AllDayHoursFixed: class extends FlagBase {
      static defaultSeverity = SEVERITY.YELLOW;
      static defaultMessage =
        'Hours were changed from 00:00-23:59 to "All Day"';
    ...
    LocalURL: class extends FlagBase {
      ...
      static venueIsFlaggable(args) {
        return (
          args.localUrlRegexString &&
          !new RegExp(args.localUrlRegexString, "i").test(args.url)
        );
      }
    },
    LockRPP: class extends ActionFlag {
      ...
      get message() {
        ...
        for (let llix = 1; llix < 6; llix++) {
          if (llix < USER.rank + 1) {
            if (
              !ddlSelected &&
              (this.args.defaultLockLevel === llix - 1 || llix === USER.rank)
            ) {
      ...
      action() {
        let levelToLock =
          $("#RPPLockLevel :selected").val() || this.args.defaultLockLevel + 1;
    ...
    AddAlias: class extends ActionFlag {
      ...
      action() {
        ...
        if (
          this.args.pnhMatch.altName2Desc &&
          !attr.description.toUpperCase().includes(alias.toUpperCase())
        ) {
    ...
    AddCat2: class extends ActionFlag {
      ...
      get message() {
        return `Is there a ${
          _catTransWaze2Lang[this.altCategory]
        } at this location?`;
      }
      get buttonTooltip() {
      ...
      static eval(args, altCategory) {
        let result = null;
        if (
          args.pnhMatch.flagsToAdd?.addCat2 &&
          !args.categories.includes(altCategory)
        ) {
    ...
    AddPharm: class extends ActionFlag {
      ...
      static venueIsFlaggable(args) {
        return (
          args.pnhMatch.flagsToAdd?.addPharm &&
          !args.categories.includes(CAT.PHARMACY)
        );
      }
    ...
    AddSuper: class extends ActionFlag {
      ...
      static venueIsFlaggable(args) {
        return (
          args.pnhMatch.flagsToAdd?.addSuper &&
          !args.categories.includes(CAT.SUPERMARKET_GROCERY)
        );
      }
    ...
    AppendAMPM: class extends ActionFlag {
      ...
      action() {
        const categories = insertAtIndex(
          this.args.venue.getCategories(),
          CAT.CONVENIENCE_STORE,
          1
        );
        addUpdateAction(
          this.args.venue,
          { name: "ARCO ampm", url: "ampm.com", categories },
          null,
          true
        );
      }
    ...
    AddATM: class extends ActionFlag {
      ...
      static venueIsFlaggable(args) {
        ...
        if (args.pnhMatch.flagsToAdd?.addATM) {
          ...
          !args.categories.includes(CAT.ATM) &&
          args.categories.includes(CAT.BANK_FINANCIAL)
        ) {
      ...
      action() {
        const categories = insertAtIndex(
          ...
          CAT.ATM,
          1
        ); // Insert ATM category in the second position
        addUpdateAction(this.args.venue, { categories }, null, true);
      }
    },
    AddConvStore: class extends ActionFlag {
      ...
      static defaultButtonText = "Yes";
      static defaultButtonTooltip =
        "Add the Convenience Store category to this place";
    ...
    ChangeToHospitalUrgentCare: class extends ActionFlag {
      ...
      static venueIsFlaggable(args) {
        return (
          !args.highlightOnly && args.categories.includes(CAT.DOCTOR_CLINIC)
        );
      }
    ...
    NotAHospital: class extends WLActionFlag {
      ...
      static venueIsFlaggable(args) {
        if (
          ...
          const testNameWords = testName.split(" ");
          return (
            containsAny(testNameWords, Pnh.HOSPITAL_FULL_MATCH) ||
            Pnh.HOSPITAL_PART_MATCH.some((match) => testName.includes(match))
          );
        }
        return false;
      }
    ...
    TitleCaseName: class extends ActionFlag {
      ...
      static venueIsFlaggable(args) {
        return (
          !args.pnhNameRegMatch && args.nameBase !== titleCase(args.nameBase)
        );
      }
      ...
      action() {
        ...
        if (name === this.#originalName || this.#confirmChange) {
          ...
          if (parts.base !== name) {
            addUpdateAction(this.args.venue, {
              name: name + (parts.suffix || ""),
            });
          }
          harmonizePlaceGo(this.args.venue, "harmonize");
        } else {
          $("button#WMEPH_titleCaseName")
            .text("Are you sure?")
            .after(" The name has changed. This will overwrite the new name.");
    ...
    NewPlaceSubmit: class extends ActionFlag {
      ...
      static defaultButtonTooltip =
        "Submit info for a new chain through the linked form";
      #formUrl;
      ...
      constructor(args) {
        ...
        const regionSettings =
          PNH_DATA[args.countryCode].regions[args.regionCode];
        let entryValues;
    ...
    ApprovalSubmit: class extends ActionFlag {
      static defaultMessage =
        "PNH data exists but is not approved for this region: ";
      static defaultButtonText = "Request approval";
      static defaultButtonTooltip =
        "Request region/country approval of this place";
      #formUrl;
    ...
    LocationFinder: class extends ActionFlag {
      static defaultButtonTooltip =
        "Look up details about this location on the chain's finder web page.";
      static #USPS_LOCATION_FINDER_URL =
      ...
      action() {
        ...
        if (
          localStorage.getItem(SETTING_IDS.sfUrlWarning) === "0" &&
          this.#isCustom
        ) {
          WazeWrap.Alerts.confirm(
            SCRIPT_NAME,
  ...
  class FlagContainer {
    ...
    static #indexFlags() {
      if (!this.#isIndexed) {
        let displayIndex = 1;
        this.#flagOrder.forEach(
          (flagClass) => (flagClass.displayIndex = displayIndex++)
        );
        this.#isIndexed = true;
      }
    }
  ...
  function harmonizePlaceGo(venue, useFlag, actions) {
    ...
    const args = new HarmonizationArgs(
      venue,
      actions,
      !useFlag.includes("harmonize")
    );
    ...
    if (
      _venueWhitelist.hasOwnProperty(venueID) &&
      (!args.highlightOnly ||
        (args.highlightOnly && !$("#WMEPH-DisableWLHL").prop("checked")))
    ) {
      ...
      Object.keys(_venueWhitelist[venueID]).forEach((wlKey) => {
        ...
        if (
          _venueWhitelist[venueID].hasOwnProperty(wlKey) &&
          (_venueWhitelist[venueID][wlKey].active || false)
        ) {
      ...
      if (
        _venueWhitelist[venueID].hasOwnProperty("dupeWL") &&
        _venueWhitelist[venueID].dupeWL.length > 0
      ) {
        if (!args.highlightOnly) _buttonBanner2.clearWL.active = true;
    ...
    if (!args.addr.country || !args.addr.state) {
      WazeWrap.Alerts.error(
        SCRIPT_NAME,
        "Country and/or state could not be determined.  Edit the place address and run WMEPH again."
      );
      return undefined;
    }
    ...
    if (
      ...
      if (!args.highlightOnly) {
        WazeWrap.Alerts.error(
          SCRIPT_NAME,
          `This script is not currently supported in ${countryName}.`
        );
      }
      return SEVERITY.RED;
    }
    ...
    for (let usdix = 1; usdix < PNH_DATA.states.length; usdix++) {
      ...
      if (stateName === _stateDataTemp[_psStateIx]) {
        ...
        if (_stateDataTemp[_psDefaultLockLevelIx].match(/[1-5]{1}/) !== null) {
          ...
          WazeWrap.Alerts.warning(
            SCRIPT_NAME,
            "Lock level sheet data is not correct"
          );
        } else {
          return 3;
        }
    ...
    if (
      !args.highlightOnly &&
      args.state2L === "TN" &&
      args.nameBase.toLowerCase().trim() === "pilot"
    ) {
    ...
    if (venue.attributes.residential) {
      ...
      args.almostAllDayHoursEntries = args.openingHours.filter(
        (hoursEntry) =>
          hoursEntry.toHour === "23:59" && /^0?0:00$/.test(hoursEntry.fromHour)
      );
      ...
      if (args.pnhNameRegMatch) {
        ...
        if (args.pnhMatch.length > 1) {
          ...
          for (
            let pnhEntryIdx = 0;
            pnhEntryIdx < args.pnhMatch.length;
            pnhEntryIdx++
          ) {
        ...
        if (nsMultiMatch && !args.highlightOnly) {
          ...
          WazeWrap.Alerts.confirm(
            ...
            () => {
              reportError({
                subject: `Order Nos. "${orderList.join(
                  ", "
                )}" WMEPH Multiple match report`,
                message: `Error report: PNH Order Nos. "${orderList.join(
                  ", "
                )}" are ambiguous multiple matches.\n \nExample Permalink: ${
                  args.placePL
                }`,
              });
            },
        ...
        if (args.pnhMatch.hasSpecialCases) {
          ...
          if (args.pnhMatch.recommendedPhone) {
            args.recommendedPhone = normalizePhone(
              args.pnhMatch.recommendedPhone,
              args.outputPhoneFormat
            );
          }
          ...
          if (
            args.pnhMatch.chainIsClosed &&
            !Flag.ChainIsClosed.isWhitelisted(args)
          ) {
        ...
        if (!args.chainIsClosed) {
          ...
          if (args.priPNHPlaceCat === CAT.HOTEL) {
            ...
            if (
              nameToCheck.toUpperCase() === args.pnhMatch.name.toUpperCase()
            ) {
              // If no localization
              args.nameBase = args.pnhMatch.name;
            } else {
              // Replace PNH part of name with PNH name
              const splix = args.nameBase
                .toUpperCase()
                .replace(/[-/]/g, " ")
                .indexOf(
                  args.pnhMatch.name.toUpperCase().replace(/[-/]/g, " ")
                );
              if (splix > -1) {
                const frontText = args.nameBase.slice(0, splix);
                const backText = args.nameBase.slice(
                  splix + args.pnhMatch.name.length
                );
                args.nameBase = args.pnhMatch.name;
            ...
            args.nameBase = args.pnhMatch.name;
            args.categories = insertAtIndex(
              args.categories,
              args.priPNHPlaceCat,
              0
            );
            if (
              altCategories?.length &&
              !args.pnhMatch.flagsToAdd?.addCat2 &&
          ...
          // Parse PNH Aliases
          if (
            !args.pnhMatch.noUpdateAlias &&
            !containsAll(args.aliases, args.pnhMatch.aliases) &&
            args.pnhMatch.aliases?.length &&
            !args.pnhMatch.optionName2
          ) {
            args.aliases = insertAtIndex(
              args.aliases,
              args.pnhMatch.aliases,
              0
            );
          ...
          const parentCats = uniq(
            args.categories.map(
              (category) => args.pnhCategoryInfos.getById(category).parent
            )
          ).filter((parent) => parent.trim().length > 0);
          args.categories = args.categories.filter(
      ...
      if (!args.chainIsClosed) {
        ...
        args.categories.forEach((category) => {
          ...
          if (
            pnhCategoryInfo.regPoint.includes(args.state2L) ||
            pnhCategoryInfo.regPoint.includes(args.regionCode) ||
            pnhCategoryInfo.regPoint.includes(args.countryCode)
          ) {
            pvaPoint = "1";
            pvaArea = "";
          } else if (
            pnhCategoryInfo.regArea.includes(args.state2L) ||
            pnhCategoryInfo.regArea.includes(args.regionCode) ||
          ...
          if (isPoint && pointSeverity > SEVERITY.GREEN) {
            args.maxPointSeverity = Math.max(
              pointSeverity,
              args.maxPointSeverity
            );
          } else if (isArea) {
          ...
          for (let lockix = 1; lockix < 6; lockix++) {
            ...
            if (
              lockix - 1 > highestCategoryLock &&
              (categoryLock.includes(args.state2L) ||
                categoryLock.includes(args.regionCode) ||
        ...
        if (!args.highlightOnly) {
          ...
          if (
            ...
            addUpdateAction(
              venue,
              { name: args.nameBase + (args.nameSuffix || "") },
              actions
            );
          }

          // Update aliases
          const tempAliases = removeUnnecessaryAliases(
            args.nameBase,
            args.aliases
          );
          if (tempAliases) {
        ...
        if (!args.highlightOnly) {
          ...
          if (!args.hoursOverlap) {
            ...
            for (let ohix = 0; ohix < args.openingHours.length; ohix++) {
              if (
                tempHours[ohix].days.length === 2 &&
                tempHours[ohix].days[0] === 1 &&
                tempHours[ohix].days[1] === 0
              ) {
                // separate hours
                ...
                tempHours.push(
                  new OpeningHour({
                    days: [0],
                    fromHour: tempHours[ohix].fromHour,
                    toHour: tempHours[ohix].toHour,
                  })
                );
          ...
          if (args.isUspsPostOffice) {
            const cleanNameParts = Flag.FormatUSPS.getCleanNameParts(
              args.nameBase,
              args.nameSuffix
            );
    ...
    if (!args.chainIsClosed) {
      ...
      const updateHnAction =
        actions &&
        actions.find(
          (action) => action.newAttributes && action.newAttributes.houseNumber
        );
      if (updateHnAction)
  ...
  function runDuplicateFinder(venue, name, aliases, addr, placePL) {
    ...
    if (
      ...
      let duplicateName = findNearbyDuplicate(
        name,
        aliases,
        venue,
        !$("#WMEPH-DisableDFZoom").prop("checked")
      );
      if (duplicateName[1]) {
      ...
      if (duplicateName.length) {
        if (duplicateName.length + 1 !== _dupeIDList.length && USER.isDevUser) {
          ...
          const wlAction = (dID) => {
            ...
            _venueWhitelist[venueID].dupeWL = uniq(
              _venueWhitelist[venueID].dupeWL
            );
  ...
  function assembleBanner(chainIsClosed) {
    ...
    _textEntryValues = $('#WMEPH_banner input[type="text"]')
      .toArray()
      .map(func);
    _textEntryValues = _textEntryValues.concat(
      $("#WMEPH_banner textarea").toArray().map(func)
    );
    ...
    Object.keys(_dupeBanner).forEach((tempKey) => {
      ...
      if (rowData.active) {
        ...
        $dupeDiv.append(
          $('<span style="margin-right:4px">').html(`&bull; ${rowData.message}`)
        );
        if (rowData.value) {
    ...
    flags.forEach((flag) => {
      ...
      switch (flag.severity) {
        ...
        default:
          throw new Error(
            `WMEPH: Unexpected severity value while building banner: ${flag.severity}`
          );
      }
      ...
      if (flag.showWL) {
        if (flag.WLaction) {
          ...
          $rowDiv.append(
            $("<button>", {
              class: "btn btn-success btn-xs wmephwl-btn",
              id: `WMEPH_WL${flag.name}`,
              title: flag.wlTooltip,
            }).text("WL")
          );
        }
      } else {
      ...
      if (flag.suffixMessage) {
        $rowDiv.append(
          $("<div>").css({ "margin-top": "2px" }).append(flag.suffixMessage)
        );
      }
    ...
    Object.keys(_buttonBanner2).forEach((tempKey) => {
      ...
      if (banner2RowData.active) {
        ...
        if (banner2RowData.action) {
          $rowDiv.append(
            ` <input class="btn btn-info btn-xs wmeph-btn" id="WMEPH_${tempKey}" title="${banner2RowData.title}" style="" type="button" value="${banner2RowData.value}">`
          );
        }
        rowDivs.push($rowDiv);
        totalSeverity = Math.max(
          _buttonBanner2[tempKey].severity,
          totalSeverity
        );
      }
    });
    ...
    if ($("#WMEPH_tools").length === 0) {
      $("#WMEPH_services").after(
        $('<div id="WMEPH_tools">').css({
    ...
    $(".wmeph-pla-spaces-btn").click((evt) => {
      ...
      const selectedValue = $(evt.currentTarget)
        ...
        .replace("wmeph_", "");
      const existingAttr =
        selectedVenue.attributes.categoryAttributes.PARKING_LOT;
      const newAttr = {};
      if (existingAttr) {
      ...
      addUpdateAction(
        selectedVenue,
        { categoryAttributes: { PARKING_LOT: newAttr } },
        null,
        true
      );
    });
  ...
  async function processGoogleLinks(venue) {
    ...
    if (googleResults.length && venue === getSelectedVenue()) {
      ...
      const googleLogoLetter = (letter, colorClass) =>
        $("<span>", { class: "google-logo" }).addClass(colorClass).text(letter);
      $bannerDiv.append(
        $("<div>", {
          ...
          .prepend(
            $("<i>", {
              ...
              style:
                "font-size: 15px;float: right;color: cadetblue;cursor: pointer;padding-left: 6px;",
            })
          )
      );
      venue.attributes.externalProviderIDs.forEach((link) => {
        ...
        if (result) {
          const linkStyle =
            "margin-left: 5px;text-decoration: none;color: cadetblue;";
          let $nameSpan;
          const $row = $("<div>", {
            class: "banner-row",
            ...
            $("<table>", { style: "width: 100%" }).append(
              $("<tbody>").append(
                $("<tr>").append(
                  ...
                  $("<td>", {
                    style:
                      "text-align: right;font-weight: 500;padding: 2px 2px 2px 0px;min-width: 65px;",
                  }).append(
                    result.website && result.requestStatus !== "NOT_FOUND"
                      ? [
                          $("<a>", {
                            ...
                            target: "_blank",
                            title:
                              "Open the place's website, according to Google",
                          }).append(
                            $("<i>", {
                          ...
                          $("<span>", {
                            style:
                              "text-align: center;margin-left: 8px;margin-right: 4px;color: #c5c5c5;cursor: default;",
                          }).text("|"),
                        ]
                      : null,
                    result.requestStatus !== "NOT_FOUND"
          ...
          if (result.requestStatus === "NOT_FOUND") {
            ...
            $row.attr(
              "title",
              "This Google place ID was not found. Please update the link in the External Providers section."
            );
            ...
            $row.attr(
              "title",
              "Google indicates this linked place is permanently closed. Please verify."
            );
            ...
            $row.attr(
              "title",
              "Google indicates this linked place is TEMPORARILY closed. Please verify."
            );
          } else if (
            googleResults.filter(
              (otherResult) => otherResult.placeId === result.placeId
            ).length > 1
          ) {
            ...
            $row.attr(
              "title",
              "This place is linked more than once. Please remove extra links."
            );
          } else {
      ...
      $("#wmeph-ext-prov-jump").click(() => {
        const extProvSelector =
          "#venue-edit-general > div.external-providers-control.form-group";
        document.querySelector(
          "#edit-panel wz-tab.venue-edit-tab-general"
        ).isActive = true;
        setTimeout(() => {
          document
            .querySelector(extProvSelector)
      ...
      return new Promise((resolve, reject) => {
        ...
        const timeoutId = setTimeout(() => {
          const error = new Error(
            `Request for place ID "${placeId}" timed out after ${
              timeoutMs / 1000
            } seconds.`
          );
  ...
  function interceptGoogleGetDetails() {
    ...
    const originalGetDetails =
      google.maps.places.PlacesService.prototype.getDetails;
    google.maps.places.PlacesService.prototype.getDetails =
      function interceptedGetDetails(request, callback) {
        ...
        const customCallback = function (result, status) {
          ...
          console.debug(
            "Intercepted getDetails response:",
            googleResult,
            status
          );
    ...
    console.debug(
      "Google Maps PlacesService.getDetails intercepted successfully."
    );
  }
  ...
  async function drawGooglePlacePoint(uuid) {
    ...
    if (link?.geometry) {
      ...
      poiPt.transform(
        W.Config.map.projection.remote,
        W.map.getProjectionObject().projCode
      );
      const placeGeom = W.selectionManager
        .getSelectedDataModelObjects()[0]
        .getOLGeometry()
        .getCentroid();
      ...
      if (splits) {
        ...
        lsLine = new OpenLayers.Geometry.LineString([
          splitPoints.components[0],
          splitPoints.components[1],
        ]);
      ...
      _googlePlacePtFeature = new OpenLayers.Feature.Vector(
        poiPt,
        { poiCoord: true },
        {
          pointRadius: 6,
      ...
      _googlePlaceLineFeature = new OpenLayers.Feature.Vector(
        lsLine,
        {},
        {
          strokeWidth: 3,
      ...
      W.map
        .getLayerByUniqueName("venues")
        .addFeatures([_googlePlacePtFeature, _googlePlaceLineFeature]);
  ...
  function timeoutDestroyGooglePlacePoint() {
    if (_destroyGooglePlacePointTimeoutId)
      clearTimeout(_destroyGooglePlacePointTimeoutId);
    _destroyGooglePlacePointTimeoutId = setTimeout(
  ...
  function assembleServicesBanner(chainIsClosed) {
    ...
    if (
      venue &&
      !chainIsClosed &&
      !$("#WMEPH-HideServicesButtons").prop("checked")
    ) {
      ...
      if (!venue.isResidential()) {
        ...
        Object.keys(_servicesBanner).forEach((tempKey) => {
          ...
          if (rowData.active) {
            //  If the particular service is active
            const $input = $("<input>", {
              class: rowData.icon,
              id: `WMEPH_${tempKey}`,
              type: "button",
              ...
              height: "27px",
              width: `${Math.ceil(
                servButtHeight * rowData.w2hratio
              ).toString()}px`,
            });
            buttons.push($input);
            if (!rowData.checked) {
        ...
        if ($rowDiv.length) {
          $rowDiv.prepend(
            '<span class="control-label" title="Verify all Place services before saving">Services (select any that apply):</span><br>'
          );
        }
        rowDivs.push($rowDiv);
      }
  ...
  function onOpenWebsiteClick() {
    ...
    try {
      ...
      WazeWrap.Alerts.error(
        SCRIPT_NAME,
        "Possible invalid URL. Check the place's Website field."
      );
    }
  }
  ...
  function onGoogleSearchClick() {
    ...
    if (addr.hasState()) {
      ...
      WazeWrap.Alerts.error(
        SCRIPT_NAME,
        "The state and country haven't been set for this place yet.  Edit the address first."
      );
    }
  }
  ...
  function updateWmephPanel(clearBanner = false) {
    ...
    if (!$("#wmeph-panel").length) {
      ...
      $googleSearchButton = $("<input>", {
        ...
        id: "wmephSearch",
        title:
          "Search the web for this place.  Do not copy info from 3rd party sources!",
        type: "button",
        value: "Google",
    ...
    if (venue) {
      ...
      $.getJSON(url).done((res) => {
        let feedNames = res.venue.external_providers
          ?.filter(
            (prov) =>
              !FEEDS_TO_SKIP.some((skipRegex) => skipRegex.test(prov.provider))
          )
        ...
        if (feedNames?.length) {
          ...
          $rowDiv.append(
            ...
            $("<div>")
              .text(
                `Place is connected to the following feed${
                  feedNames.length > 1 ? "s" : ""
                }:`
              )
              .css({ "font-size": "13px" }),
  ...
  function findNearbyDuplicate(
    ...
    let currNameList = [];
    if (
      selectedVenueNameRF.length > 2 ||
      allowedTwoLetters.includes(selectedVenueNameRF)
    ) {
    ...
    if (
      ((venueNameNoNum.length > 2 && !NO_NUM_SKIP.includes(venueNameNoNum)) ||
        allowedTwoLetters.includes(venueNameNoNum)) &&
    ...
    if (selectedVenueAliases.length > 0) {
      for (let aliix = 0; aliix < selectedVenueAliases.length; aliix++) {
        ...
        if (
          (aliasNameRF.length > 2 && !NO_NUM_SKIP.includes(aliasNameRF)) ||
          allowedTwoLetters.includes(aliasNameRF)
        ) {
          // only add de-numbered name if anything remains
        ...
        if (
          ((aliasNameNoNum.length > 2 &&
            !NO_NUM_SKIP.includes(aliasNameNoNum)) ||
            allowedTwoLetters.includes(aliasNameNoNum)) &&
          !selectedVenueAttr.categories.includes(CAT.PARKING_LOT)
        ) {
          // only add de-numbered name if anything remains
    ...
    const whitelistedDupes =
      selectedVenueWL && selectedVenueWL.dupeWL ? selectedVenueWL.dupeWL : [];
      ...
      if (
        ...
        if (
          ...
          if (!suppressMatch) {
            ...
            const strippedTestName = formatName(testVenueAttr.name).replace(
              /\s+[-(].*$/,
              ""
            ); // Remove localization text
            if (
              (strippedTestName.length > 2 &&
                !NO_NUM_SKIP.includes(strippedTestName)) ||
            ...
            if (
              ((testNameNoNum.length > 2 &&
                !NO_NUM_SKIP.includes(testNameNoNum)) ||
                allowedTwoLetters.includes(testNameNoNum)) &&
              !testVenueAttr.categories.includes(CAT.PARKING_LOT)
            ) {
              //  only add de-numbered name if at least 2 chars remain
            ...
            for (let tnlix = 0; tnlix < testNameList.length; tnlix++) {
              for (let cnlix = 0; cnlix < currNameList.length; cnlix++) {
                if (
                  testNameList[tnlix].includes(currNameList[cnlix]) ||
                  currNameList[cnlix].includes(testNameList[tnlix])
                ) {
                  nameMatch = true;
                  break;
                }
            ...
            if (!nameMatch && testVenueAttr.aliases.length > 0) {
              for (
                let aliix = 0;
                aliix < testVenueAttr.aliases.length;
                aliix++
              ) {
                const aliasNameRF = formatName(testVenueAttr.aliases[aliix]);
                if (
                  (aliasNameRF.length > 2 &&
                    !NO_NUM_SKIP.includes(aliasNameRF)) ||
                  allowedTwoLetters.includes(aliasNameRF)
                ) {
                ...
                if (
                  ...
                  //  only add de-numbered name if at least 2 characters remain
              ...
              for (let tnlix = 0; tnlix < testNameList.length; tnlix++) {
                ...
                if (altNameMatch > -1) {
                  break;
                } // break from the rest of the alts if a match found
              }
            }
            // If a match was found:
            if (nameMatch || altNameMatch > -1) {
              ...
              const labelText = nameMatch
                ? testVenueAttr.name
                : `${testVenueAttr.aliases[altNameMatch]} (Alt)`;
              logDev(
                `Possible duplicate found. WME place: ${selectedVenueName} / Nearby place: ${labelText}`
              );

              // Reformat the name into multiple lines based on length
              const labelTextBuild = [];
              let maxLettersPerLine = Math.round(
                2 * Math.sqrt(labelText.replace(/ /g, "").length / 2)
              );
              ...
              while (endIX !== -1) {
                ...
                if (endIX - startIX > maxLettersPerLine) {
                  labelTextBuild.push(
                    labelText.substr(startIX, endIX - startIX)
                  );
                  startIX = endIX + 1;
                }
              }
              ...
              labelFeatures.push(
                new OpenLayers.Feature.Vector(testCentroid, {
                  labelText: labelTextReformat,
                  fontColor: "#fff",
                  strokeColor:
                    labelColorList[labelColorIX % labelColorList.length],
                  labelAlign: "cm",
                  pointRadius: 25,
                  dupeID: testVenueId,
                })
    ...
    if (_dupeIDList.length > 1) {
      ...
      labelFeatures.push(
        new OpenLayers.Feature.Vector(selectedCentroid, {
    ...
    if (recenterOption && dupeNames.length > 0 && outOfExtent) {
      // then rebuild the extent to include the duplicate
      const padMult = 1.0;
  ...
  function inferAddress(venue, maxRecursionDepth) {
    ...
    const findConnections = (startingNodeID, recursionDepth) => {
      ...
      const connectedSegments = orderedSegments.filter((seg) =>
        [seg.fromNodeID, seg.toNodeID].includes(startingNodeID)
      );
      ...
      for (let idx = 0; idx < keys.length; idx++) {
        ...
        if (hasStreetName(connectedSegments[k].segment)) {
          ...
          newNode =
            attr.fromNodeID === startingNodeID
              ? attr.toNodeID
              : attr.fromNodeID;
    ...
    if (hasStreetName(orderedSegments[0].segment)) {
      ...
      if (foundAddresses.length > 0) {
        ...
        const closestElem = orderedSegments.find((element) =>
          hasStreetName(element.segment)
        );
        inferredAddress = closestElem
          ? closestElem.segment.getAddress() || inferredAddress
          : inferredAddress;
      }
    }
  ...
  function phoneFormat(format, ...rest) {
    return format.replace(/{(\d+)}/g, (name, number) =>
      typeof rest[number] !== "undefined" ? rest[number] : null
    );
  }
  ...
  function initShortcutKey() {
    ...
    if (localStorage.getItem("WMEPH-KBSModifierKey") === "1") {
  ...
  function onWLMergeClick() {
    ...
    if ($wlInput.val() === "resetWhitelist") {
      ...
      if (wlStringToMerge) {
        ...
        // try compressed WL
        wlStringToMerge = validateWLS(
          LZString.decompressFromUTF16($("#WMEPH-WLInput").val())
        );
        if (wlStringToMerge) {
          log("Whitelists merged!");
          _venueWhitelist = mergeWL(_venueWhitelist, wlStringToMerge);
    ...
    $("#PlaceHarmonizerWLToolsMsg")
      ...
      .append(
        $("<div>", { style: "margin-bottom: 10px;" }).text(
          `Number of WL places: ${entries.length}`
        ),
  ...
  function onWLStateFilterClick() {
    ...
    if (stateToRemove.length < 2) {
      msgColor = "red";
      msgText =
        'Invalid state. Enter the state name in the "Whitelist string" box above, ' +
      ...
      const venuesToRemove = Object.keys(currWLData).filter(
        (venueKey) =>
          venueKey !== "1.1.1" &&
          (currWLData[venueKey].state === stateToRemove ||
      ...
      if (venuesToRemove.length > 0) {
            ...
            () => {
              ...
              $("#PlaceHarmonizerWLToolsMsg")
                ...
                .append(
                  $("<p>")
                    .css({ color: "green" })
                    .text(`${venuesToRemove.length} venues removed from WL`)
                );
            },
            () => {
              $("#PlaceHarmonizerWLToolsMsg")
                .empty()
                .append(
                  $("<p>").css({ color: "blue" }).text("No changes made")
                );
            }
          );
          return;
        ...
        msgColor = "red";
        msgText =
          "Please backup your WL using the Pull button before removing state data";
        // }
      } else {
        msgColor = "red";
  ...
  function onWLShareClick() {
    window.open(
      `https://docs.google.com/forms/d/1k_5RyOq81Fv4IRHzltC34kW3IUbXnQqDVMogwJKFNbE/viewform?entry.1173700072=${USER.name}`
    );
  }
  ...
  function initWmephTab() {
    ...
    if (!_initAlreadyRun) {
      const runOnceDefaultIgnorePlaGoogleLinkChecks = localStorage.getItem(
  ...
  async function addWmephTab() {
    ...
    const $versionDiv = $("<div>")
      .text(`WMEPH ${BETA_VERSION_STR} v${SCRIPT_VERSION}`)
      .css({ color: "#999", fontSize: "13px" });
    const $harmonizerTab = $(
      '<div class="tab-pane wmeph-pane active" id="sidepanel-harmonizer"></div>'
    );
    const $highlighterTab = $(
      '<div class="tab-pane wmeph-pane" id="sidepanel-highlighter"></div>'
    );
    const $wlToolsTab = $(
      '<div class="tab-pane wmeph-pane" id="sidepanel-wltools"></div>'
    );
    const $moderatorsTab = $(
      '<div class="tab-pane wmeph-pane" id="sidepanel-pnh-moderators"></div>'
    );
    $tabContent.append(
      $harmonizerTab,
      $highlighterTab,
    ...
    createSettingsCheckbox(
      $harmonizerTab,
      "WMEPH-WebSearchNewTab",
      "Open URL & Search Results in new tab instead of new window"
    );
    createSettingsCheckbox(
      $harmonizerTab,
      "WMEPH-DisableDFZoom",
      "Disable zoom & center for duplicates"
    );
    createSettingsCheckbox(
      $harmonizerTab,
      "WMEPH-EnableIAZoom",
      "Enable zoom & center for places with no address"
    );
    createSettingsCheckbox(
      $harmonizerTab,
      "WMEPH-HidePlacesWiki",
      'Hide "Places Wiki" button in results banner'
    );
    createSettingsCheckbox(
      $harmonizerTab,
      "WMEPH-HideReportError",
      'Hide "Report script error" button in results banner'
    );
    createSettingsCheckbox(
      $harmonizerTab,
      "WMEPH-HideServicesButtons",
      "Hide services buttons in results banner"
    );
    createSettingsCheckbox(
      $harmonizerTab,
      "WMEPH-HidePURWebSearch",
      'Hide "Web Search" button on PUR popups'
    );
    createSettingsCheckbox(
      $harmonizerTab,
      "WMEPH-ExcludePLADupes",
      "Exclude parking lots when searching for duplicate places"
    );
    createSettingsCheckbox(
      $harmonizerTab,
      "WMEPH-ShowPLAExitWhileClosed",
      "Always ask if cars can exit parking lots"
    );
    if (USER.isDevUser || USER.isBetaUser || USER.rank >= 2) {
      createSettingsCheckbox(
        $harmonizerTab,
        "WMEPH-DisablePLAExtProviderCheck",
        'Disable check for "Google place link" on Parking Lot Areas'
      );
      createSettingsCheckbox(
        $harmonizerTab,
        "WMEPH-AddAddresses",
        "Add detected address fields to places with no address"
      );
      createSettingsCheckbox(
        $harmonizerTab,
        "WMEPH-EnableCloneMode",
        "Enable place cloning tools"
      );
      createSettingsCheckbox(
        $harmonizerTab,
        "WMEPH-AutoLockRPPs",
        "Lock residential place points to region default"
      );
    }
    ...
    if (USER.isDevUser) {
      ...
      $phShortcutDiv.append(
        '<hr class="wmeph-hr" align="center" width="100%"><p>Dev Only Settings:</p>'
      );
      createSettingsCheckbox(
        $phShortcutDiv,
        "WMEPH-RegionOverride",
        "Disable Region Specificity"
      );
    }
    ...
    createSettingsCheckbox(
      $highlighterTab,
      "WMEPH-ColorHighlighting",
      "Enable color highlighting of map to indicate places needing work"
    );
    createSettingsCheckbox(
      $highlighterTab,
      "WMEPH-DisableHoursHL",
      "Disable highlighting for missing hours"
    );
    createSettingsCheckbox(
      $highlighterTab,
      "WMEPH-DisableRankHL",
      "Disable highlighting for places locked above your rank"
    );
    createSettingsCheckbox(
      $highlighterTab,
      "WMEPH-DisableWLHL",
      "Disable Whitelist highlighting (shows all missing info regardless of WL)"
    );
    createSettingsCheckbox(
      $highlighterTab,
      "WMEPH-PLATypeFill",
      "Fill parking lots based on type (public=blue, restricted=yellow, private=red)"
    );
    createSettingsCheckbox(
      $highlighterTab,
      "WMEPH-ShowFilterHighlight",
      "Highlight places without Customer Parking service"
    );
    ...
    $moderatorsTab.append(
      $("<div>", { style: "margin-bottom: 10px;" }).text(
        "Moderators are responsible for reviewing chain submissions for their region." +
    ...
    tabLabel.innerHTML = `<span title="WME Place Harmonizer">WMEPH${
      IS_BETA_VERSION ? "-β" : ""
    }</span>`;
  ...
  function createCloneCheckbox(divID, settingID, textDescription) {
    ...
    const $label = $("<label>", {
      for: settingID,
      style: "margin-left: 2px; font-weight: normal",
    }).text(textDescription);

    return $("<span>", { style: "margin-right: 6px;" }).append(
      $checkbox,
      $label
    );
  }
          ...
          if (wlVenue2.hasOwnProperty(wlKey) && wlVenue2[wlKey].active) {
            ...
            if (wlVenue1.hasOwnProperty(wlKey) && wlVenue1[wlKey].active) {
              if (wlVenue1[wlKey].hasOwnProperty("WLKeyArray")) {
                wl1[venueKey][wlKey].WLKeyArray = insertAtIndex(
                  wl1[venueKey][wlKey].WLKeyArray,
                  wl2[venueKey][wlKey].WLKeyArray,
                  100
                );
              }
            } else {
    ...
    if (venue) {
      ...
      Object.keys(_servicesBanner).forEach((keys) => {
        if (_servicesBanner.hasOwnProperty(keys)) {
          ...
          _servicesBanner[keys].active =
            _servicesBanner[keys].active || servArrayCheck[wsix]; // display any manually checked non-active icons
          wsix++;
        }
      });
      // Highlight 24/7 button if hours are set that way, and add button for all places
  ...
  function updateUserInfo() {
    ...
    if (!_wmephBetaList || _wmephBetaList.length === 0) {
      if (IS_BETA_VERSION) {
        WazeWrap.Alerts.warning(
          SCRIPT_NAME,
          "Beta user list access issue.  Please post in the GHO or PM/DM MapOMatic about this message.  Script should still work."
        );
      }
  ...
  function onWindowBeforeUnload() {
    localStorage.setItem(
      ...
      getShortcutKeys(W.accelerators.Actions.wmephFilterHighlightToggle)
    );
    localStorage.setItem(
      "WMEPH_ColorHighlighting",
      getShortcutKeys(W.accelerators.Actions.wmephColorHighlightingToggle)
    );
  }
  ...
  async function placeHarmonizerInit() {
    ...
    const downloadUrl = IS_BETA_VERSION
      ? dec(BETA_DOWNLOAD_URL)
      : PROD_DOWNLOAD_URL;
    let updateMonitor;
    try {
    ...
    if (!_dupeLayer) {
      ...
      _dupeLayer = new OpenLayers.Layer.Vector(lname, {
    ...
    if (
      ...
      if (validateWLS(localStorage.getItem(WL_LOCAL_STORE_NAME)) === false) {
        ...
        localStorage.setItem(
          "WMEPH-OneTimeWLBU",
          localStorage.getItem(WL_LOCAL_STORE_NAME)
        );
        loadWhitelistFromLS(false);
        saveWhitelistToLS(true);
        WazeWrap.Alerts.info(
          SCRIPT_NAME,
          "Whitelists are being converted to a compressed format.  If you have trouble with your WL, please submit an error report."
        );
      }
    } else {
    ...
    if (USER.name === "ggrane") {
      _searchResultsWindowSpecs = `"resizable=yes, top=${Math.round(
        window.screen.height * 0.1
      )}, left=${Math.round(window.screen.width * 0.3)}, width=${Math.round(
    ...
    if (removedWLCount > 0) {
      saveWhitelistToLS(true);
      logDev(
        `Removed ${removedWLCount} venues with temporary ID's from WL store`
      );
    }
  ...
  function processFilterHighlights() {
    ...
    W.model.venues
      .getObjectArray(
        (v) =>
          !v.attributes.services.includes("PARKING_FOR_CUSTOMERS") &&
          !CATS_TO_IGNORE_CUSTOMER_PARKING_HIGHLIGHT.includes(
            v.attributes.categories[0]
          )
      )
      .forEach((v) => {
        let style;
        ...
        const f = new OpenLayers.Feature.Vector(
          geometry,
          { wmephHighlight: "1" },
          style
        );
  ...
  async function bootstrap() {
    ...
    if (unsafeWindow.wmephRunning) {
      ...
      alert(
        "Multiple versions of WME Place Harmonizer are turned on. Only one will be enabled."
      );
      return;
    }
