...
(function main() {
    ...
    const GENERAL_SERVICES = ['VALLET_SERVICE', 'DRIVETHROUGH', 'WI_FI', 'RESTROOMS', 'CREDIT_CARDS', 'RESERVATIONS', 'OUTSIDE_SEATING',
        'AIR_CONDITIONING', 'PARKING_FOR_CUSTOMERS', 'DELIVERIES', 'TAKE_AWAY', 'CURBSIDE_PICKUP', 'WHEELCHAIR_ACCESSIBLE'];
    const PARKING_LOT_SERVICES = ['AIRPORT_SHUTTLE', 'CAR_WASH', 'CARPOOL_PARKING', 'COVERED', 'DISABILITY_PARKING', 'EV_CHARGING_STATION',
        'ON_SITE_ATTENDANT', 'PARK_AND_RIDE', 'RESERVATIONS', 'SECURITY', 'VALET', 'VALLET_SERVICE'];
    ...
    function getServicesBanner() {
        ...
        return {
            ...
            addAirportShuttle: {
                ...
                pnhOverride: false,
                actionOn(actions) { this.action(actions, true); },
                actionOff(actions) { this.action(actions, false); }
            },
            addCarWash: {
                active: true,
                checked: false,
    ...
    function harmonizePlaceGo(venue, useFlag, actions) {
        ...
        if (!args.chainIsClosed) {
            ...
            if (args.regionCode === 'SER') {
                ...
                } else if (isVenuePoint(venue) && args.categories.includes(CAT.COLLEGE_UNIVERSITY) && (!args.categories.includes(CAT.HOSPITAL_MEDICAL_CARE)
        ...
        Object.keys(_buttonBanner2).forEach(tempKey => {
            ...
            if (banner2RowData.active) { //  If the particular message is active
                if (banner2RowData.action) {
                    $buttonContainer.append(` <input class="btn btn-info btn-xs wmeph-clone-btn" id="WMEPH_${tempKey}" title="${
                        banner2RowData.title}" type="button" value="${banner2RowData.value}" style="font-weight: normal; margin-left: 3px;">`);
                }
        ...
        if (venue && !chainIsClosed && !$('#WMEPH-HideServicesButtons').prop('checked')) {
            ...
            if (!isVenueResidential(venue)) {
                ...
                Object.keys(_servicesBanner).forEach(tempKey => {
                    ...
                    const isBtnForWrongVenueType = (isParkingLot && (tempKey === 'addValet' || tempKey === 'addReservations')) ||
                                                    (!isParkingLot && (tempKey === 'addValetPL' || tempKey === 'addReservationsPL' || tempKey === 'addValletServicePL'));
                    if (rowData.active && isApplicable && !isBtnForWrongVenueType) { //  If the particular service is active AND applicable to this venue type
                        const $input = $('<input>', {
                            class: rowData.icon,
                            id: `WMEPH_${tempKey}`,
                            type: 'button',
