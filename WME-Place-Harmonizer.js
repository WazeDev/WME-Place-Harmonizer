...
// @downloadURL https://update.greasyfork.org/scripts/28689/WME%20Place%20Harmonizer%20Beta.user.js
// @updateURL https://update.greasyfork.org/scripts/28689/WME%20Place%20Harmonizer%20Beta.meta.js
// ==/UserScript==
...
(function main() {
    ...
    const wmephVenueData = new Map();

    function setVenueData(venueId, props) {
        wmephVenueData.set(venueId, {
            ...wmephVenueData.get(venueId),
            ...props
        });
    }

    // BE SURE TO SET THIS TO NULL OR AN EMPTY STRING WHEN RELEASING A NEW UPDATE.
    const _SCRIPT_UPDATE_MESSAGE = "Bringing back some highlighting...it's not super reliable and whitelisting doesn't seem to work.";
    const _CSS = `
...
function whitelistAction(venueID, wlKeyName) {
    const venue = getSelectedVenue();
    let addressTemp = venue.getAddress(W.model);
...
function toggleXrayMode(enable) {
    ...
    sdk.Map.addStyleRuleToLayer({
        ...
        styleRules: [
            {
                predicate: (properties) => wmephVenueData.get(String(properties.id))?.wmephSeverity === 0,
                style: {
                    strokeWidth: 1.67,
                    strokeColor: '#888',
                    pointRadius: 5,
            ...
            {
                predicate: (properties) => wmephVenueData.get(String(properties.id))?.wmephSeverity === 'lock',
                style: {
                    strokeColor: 'white',
                    fillColor: '#080',
                    fillOpacity: 1,
            ...
            {
                predicate: (properties) => wmephVenueData.get(String(properties.id))?.wmephSeverity === 1,
                style: {
                    strokeColor: 'white',
                    strokeWidth: 2,
                    externalGraphic: '',
            ...
            {
                predicate: (properties) => wmephVenueData.get(String(properties.id))?.wmephSeverity === 'lock1',
                style: {
                    pointRadius: defaultPointRadius,
                    fillColor: '#0055ff',
                    strokeColor: 'white',
            ...
            {
                predicate: (properties) => wmephVenueData.get(String(properties.id))?.wmephSeverity === 2,
                style: {
                    pointRadius: defaultPointRadius,
                    fillColor: '#ca0',
                    strokeColor: 'white',
            ...
            {
                predicate: (properties) => wmephVenueData.get(String(properties.id))?.wmephSeverity === 3,
                style: {
                    strokeColor: 'white',
                    strokeWidth: 2,
                    pointRadius: defaultPointRadius,
            ...
            {
                predicate: (properties) => wmephVenueData.get(String(properties.id))?.wmephSeverity === 4,
                style: {
                    fillColor: '#f42',
                    strokeLinecap: 1,
                    strokeWidth: 2,
            ...
            {
                predicate: (properties) => wmephVenueData.get(String(properties.id))?.wmephSeverity === 5,
                style: {
                    fillColor: 'black',
                    strokeColor: '#f4a',
                    strokeLinecap: 1,
            ...
            {
                predicate: (properties) => wmephVenueData.get(String(properties.id))?.wmephSeverity === 'adLock',
                style: {
                    pointRadius: 12,
                    fillColor: 'yellow',
                    fillOpacity: 0.4,
...
function initializeHighlights() {
    ...
    sdk.Map.addStyleRuleToLayer({
        ...
        styleRules: [
            ...
            {
                predicate: (properties) => wmephVenueData.get(String(properties.id))?.wmephSeverity === 6,
                style: {
                    strokeColor: '#f80',
                    strokeWidth: 6,
                    pointRadius: 10,
            ...
            },
            {
                // Public parking lot
                predicate: (properties) => {
                    if (!properties.id || !$('#WMEPH-PLATypeFill').prop('checked')) return false;
                    const venue = sdk.DataModel.Venues.getById({ venueId: String(properties.id) });
                    if (!venue?.categories?.includes('PARKING_LOT')) return false;
                    const type = venue.categoryAttributes?.PARKING_LOT?.parkingType;
                    return !type || type.toLowerCase() === 'public';
                },
                style: {
                    fillColor: '#0000FF',
                    fillOpacity: 0.25
            ...
            },
            {
                ...
                predicate: (properties) => {
                    ...
                    const venue = sdk.DataModel.Venues.getById({ venueId: String(properties.id) });
                    if (!venue?.categories?.includes('PARKING_LOT')) return false;
                    return venue.categoryAttributes?.PARKING_LOT?.parkingType?.toLowerCase() === 'restricted';
                },
                style: {
                    fillColor: '#FFFF00',
                    fillOpacity: 0.3
            ...
            },
            {
                ...
                predicate: (properties) => {
                    ...
                    const venue = sdk.DataModel.Venues.getById({ venueId: String(properties.id) });
                    if (!venue?.categories?.includes('PARKING_LOT')) return false;
                    return venue.categoryAttributes?.PARKING_LOT?.parkingType?.toLowerCase() === 'private';
                },
                style: {
                    fillColor: '#FF0000',
                    fillOpacity: 0.25
...
function applyHighlightsTest(venues, force) {
        ...
        if (venue && venue.type === 'venue' && venue.attributes) {
            ...
            if (doHighlight && !(disableRankHL && venue.attributes.lockRank > USER.rank - 1)) {
                try {
                    ...
                    setVenueData(id, { wmephSeverity: severity});
                } catch (err) {
                    console.error('WMEPH highlight error: ', err);
                }
            } else {
                //venue.attributes.wmephSeverity = 'default';
                setVenueData(venue.attributes.id, {wmephSeverity: 'default'});
            }
        }
    });
    ...
    if (venue) {
        ...
        let severity = harmonizePlaceGo(venue, 'highlight');
        setVenueData(venue.attributes.id, {wmephSeverity: severity});
    ...
    PlaNameNonStandard: class extends WLFlag {
        ...
        static venueIsFlaggable(args) {
            if (!this.isWhitelisted(args) && args.venue.isParkingLot()) {
                ...
                if (name) {
                    const state = args.venue.getAddress(W.model).getStateName(W.model);
            ...
            action() {
                reportError({
                    ...
                    message: `Script version: ${SCRIPT_VERSION}${BETA_VERSION_STR}\nPermalink: ${
                            ...
                            venue.getAddress(W.model).getCountry().name}\n--------\nDescribe the error:  \n `
                    });
            }
        }
    };
...
function assembleBanner(chainIsClosed) {
    ...
    if ($('#WMEPH-ColorHighlighting').prop('checked')) {
        ...
        debugger;
        setVenueData(venue.attributes.id, {wmephSeverity: totalSeverity});
    }
...
function onCopyClicked() {
    ...
    _cloneMaster = {};
    _cloneMaster.addr = venue.getAddress(W.model);
...
function onGoogleSearchClick() {
    const venue = getSelectedVenue();
    const addr = venue.getAddress(W.model);
    if (addr.hasState()) {
...
function clonePlace() {
    ...
    if (_cloneMaster !== null && _cloneMaster.hasOwnProperty('url')) {
        ...
        if (copyStreet || copyCity || copyHn) {
            const originalAddress = venue.getAddress(W.model);
            const newAddress = {
...
function buildGLink(searchName, addr, HN) {
    ...
    searchName = searchName + (searchName ? ', ' : '') + searchHN + searchStreet
        + searchCity + addr.getStateName(W.model);
...
async function placeHarmonizerInit() {
    ...
    // re-scan venues and repopulate
    sdk.Events.on({
        eventName: 'wme-map-data-loaded',
        eventHandler: () => {
            wmephVenueData.clear();
        }
    });
